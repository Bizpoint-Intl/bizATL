...
namespace ATL.WRR
{
    public class Voucher_WRR : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        public Voucher_WRR(string moduleName, Hashtable voucherBaseHelpers,string Sector)
        ...
        protected override void Document_SaveBegin_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            foreach (DataRow dr1 in wrr1.Rows)
            {
                if (dr1.RowState != DataRowState.Deleted)
                {
                    BizFunctions.UpdateDataRow(wrr, dr1, "refnum/sitenum/weeknum/user/flag/status/created/modified");
            ...
            foreach (DataRow dr2 in wrr2.Rows)
            {
                if (dr2.RowState != DataRowState.Deleted)
                {
                    BizFunctions.UpdateDataRow(wrr, dr2, "refnum/sitenum/weeknum/user/flag/status/created/modified");
            ...
            foreach (DataRow dr3 in wrr3.Rows)
            {
                if (dr3.RowState != DataRowState.Deleted)
                {
                    BizFunctions.UpdateDataRow(wrr, dr3, "refnum/sitenum/weeknum/user/flag/status/created/modified");
                }
            ...
            foreach (DataRow dr4 in wrr4.Rows)
            {
                if (dr4.RowState != DataRowState.Deleted)
                {
                    BizFunctions.UpdateDataRow(wrr, dr4, "refnum/sitenum/weeknum/user/flag/status/created/modified");
                }
            }
                    ...
                    if (!BizFunctions.IsEmpty(wrr["adhocrefnum"]))
                    {
                        ...
                        if (Convert.ToDateTime(wrr["commencedate"]).ToShortDateString() == Convert.ToDateTime(e.F2CurrentRow["adhocFrom"]).ToShortDateString() && Convert.ToDateTime(wrr["enddate"]).ToShortDateString() == Convert.ToDateTime(e.F2CurrentRow["adhocTo"]).ToShortDateString())
                        {
                            ...
                            if (!BizFunctions.IsEmpty(wrr["adhocrefnum"].ToString().Trim()) || wrr["adhocrefnum"].ToString().Trim() != string.Empty)
                            {
                                ...
                                {
                                    MessageBox.Show("Contract No. Doesn't Exist!", "ASOMS", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    return;
                                }
                        ...
                        {
                            MessageBox.Show("Can't Add Schedule which is different from the Header Schedule!", "ASOMS", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                Query = "Select " +
                            ...
                            "( " +
                                "select " +
                                    ...
                                    "ISNULL(officerqty,0) * ISNULL(sunday,0) as sunday " +
                                    "from ADH1 " +
                                    "where refnum='" + wrr["adhocrefnum"].ToString().Trim() + "' and  shifttype='" + ShiftType + "' and adhocFrom>='" + BizFunctions.GetSafeDateString(Convert.ToDateTime(wrr["commencedate"])) + "' and adhocTo<='" + BizFunctions.GetSafeDateString(Convert.ToDateTime(wrr["enddate"])) + "' " +
                            ")a";
            }
        ...
        private void updateATMR()
        {
            ...
            string FinalSchedule = "Select FW.REFNUM,FW.TB,FW.EMPNUM,FW.NRIC,FW.SHIFTCODE,FW.[DAY],FW.TIMEIN,FW.[TIMEOUT],FB.[DATE], FW.ISOT,FW.OTRATE,FW.ISDRE,FW.ISOFFSET,FW.ISUS,FW.ISRD,FW.REMARK,FW.isRealSchedule FROM FinalAMTR FW " +
