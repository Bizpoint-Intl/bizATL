...
namespace PicoGuards.WRR
{
    public class Voucher_WRR : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        protected override void Document_Save_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            if (!this.dbaccess.DataSet.Tables.Contains("hemph"))
            {
                string Get = "Select * from hemph where [status]<>'V'";
                this.dbaccess.ReadSQL("HEMPH",Get);
            }
            else if (hemph.Rows.Count <= 0)
            {
                string Get = "Select * from hemph where [status]<>'V'";
        ...
        private void updateATMR()
        {
            ...
            string FinalSchedule = "Select FW.REFNUM,FW.TB,FW.EMPNUM,FW.NRIC,FW.SHIFTCODE,FW.[DAY],FW.TIMEIN,FW.[TIMEOUT],FB.[DATE],FW.REMARK FROM FinalAMTR FW " +
        ...
        {
            string sql1 = "";
            sql1 = "SELECT M1.refnum,M1.TB,M1.EMPNUM,M1.NRIC, M1.shiftcode, [day], vS.TIMEIN,vS.[TIMEOUT],M1.remark " +
                "FROM " +
                "( " +
                    "Select unP.refnum,TB,unP.empnum,unP.nric,shiftcode,[day],remark from " +
                    "( " +
                        "select refnum,empnum,nric,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday,Sunday,remark,'wrr1' as TB from wrr1 where refnum='" + refnum + "'" +
                    ") as p " +
                    "UNPIVOT " +
                    "( " +
                    ...
                    "Select unP.refnum,TB,unP.empnum,unP.nric,shiftcode,[day],remark  from " +
                    "( " +
                        "select refnum,empnum,nric,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday,Sunday,remark,'wrr2' as TB  from wrr2 where refnum='" + refnum + "'" +
                    ") as p " +
                    "UNPIVOT " +
                    "( " +
                        "shiftcode for [day] in (MONDAY,TUESDAY,WEDNESDAY,THURSDAY,FRIDAY,SATURDAY,SUNDAY) " +
                    ...
                    "Select unP.refnum,TB,unP.empnum,unP.nric,shiftcode,[day],remark  from " +
                    "( " +
                       "select refnum,empnum,nric,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday,Sunday,remark,'wrr3' as TB  from wrr3 where refnum='" + refnum + "'" +
                    ") as p " +
                    "UNPIVOT " +
                    "( " +
                        "shiftcode for [day] in (MONDAY,TUESDAY,WEDNESDAY,THURSDAY,FRIDAY,SATURDAY,SUNDAY) " +
                    ...
                    "Select unP.refnum,TB,unP.empnum,unP.nric,shiftcode,[day],remark  from " +
                    "( " +
                        "select refnum,empnum,nric,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday,Sunday,remark,'wrr4' as TB  from wrr4 where refnum='" + refnum + "'" +
                    ") as p " +
                    "UNPIVOT " +
                    "( " +
                        "shiftcode for [day] in (MONDAY,TUESDAY,WEDNESDAY,THURSDAY,FRIDAY,SATURDAY,SUNDAY) " +
        ...
        //private int GetTotalReliefNight(string Day, string TableName)
        //{
        //    int NightTotal = 0;
        ...
        //private int GetTotalReliefConcierge(string Day, string TableName)
        //{
        //    int ConciergeTotal = 0;

        //    return ConciergeTotal;
        //}
