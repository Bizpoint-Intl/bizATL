...
namespace ATL.TRQ
{
    public class Voucher_TRQ : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        private decimal previousQtyRequest(string matnum)
        {
            ...
            string getPreviousQty = "select t1.matnum, sum(t1.qty) as previousQtyRequest " +
                                    "from tri1  t1 "+
                                            "left join trih th on t1.refnum=th.refnum " +
                                    "where t1.tositenum='" + trqh["tositenum"].ToString().Trim() + "' and DATEADD(dd,-14,Convert(Datetime,'" + BizFunctions.GetSafeDateString(Convert.ToDateTime(trqh["trandate"])) + "'))<=th.trandate and t1.matnum='"+matnum+"' group by t1.matnum";
