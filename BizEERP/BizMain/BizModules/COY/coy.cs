...
namespace ATL.Coy
{
	public class COY
	{
		...
		public COY()
		{
			...
			Form form = BizXmlReader.CurrentInstance.Load(this.projectPath+@"\COY\coy.xml", "coy", this, null) as Form;

			this.dbAccess			= new DBAccess();
			this.selectsCollection	= new Hashtable();

			DataGrid dataGrid = (DataGrid)BizXmlReader.CurrentInstance.GetControl("coy", "dataGrid");

			this.selectsCollection.Add("coy", "SELECT * FROM coy");
			this.selectsCollection.Add("SysLocker", "SELECT * FROM SysLocker WHERE ModuleName='COY' AND [Key]='COY'");
			this.dbAccess.ReadSQL(selectsCollection);

			if(this.dbAccess.DataSet.Tables["SysLocker"].Rows.Count != 0)
			{
				MessageBox.Show("Company Settings is locked/in use !", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Information);
			}
			else
			{
				this.dbAccess.RemoteStandardSQL.ExecuteNonQuery("INSERT INTO SysLocker (ModuleName, [Key], UserName) VALUES ('COY','COY','"+Common.DEFAULT_SYSTEM_USERNAME+"')");
				this.ds = this.dbAccess.DataSet;

				BizGridInfo detailGridInfo = (BizGridInfo)BizXmlReader.CurrentInstance.Load(this.projectPath+@"\COY\DetailGridInfo_Coy.xml", "detailGridInfo", null, null);
		...
		protected void btn_Update_Click(object sender, System.EventArgs e)
		{
			if (MessageBox.Show("Confirm Update Changes to Database?", "Warning", MessageBoxButtons.YesNo) == DialogResult.Yes)
			{
				...
				foreach(DataRow dr in coy.Rows)
				{
				...
				savetable[0] = coy.GetChanges();
				if(savetable[0] != null)
				{
                    savetable[0].TableName = "coy";
					savetable[0].Columns.Remove("coyimage");
		...
		{
			Form form = BizXmlReader.CurrentInstance.GetForm("coy") as Form;
			form.Close();
		}
		...
		{
						this.dbAccess.RemoteStandardSQL.ExecuteNonQuery("DELETE FROM SysLocker WHERE ModuleName='COY' AND [Key]='COY'");
		}

		#endregion

        #region Insert Clicked
