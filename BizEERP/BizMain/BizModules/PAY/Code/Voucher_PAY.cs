...
namespace ATL.PAY
{
	public class Voucher_PAY : BizRAD.BizApplication.VoucherBaseHelper
	{
		...
		protected override void Document_Reopen_Handle(object sender, DocumentHandleEventArgs e)
		{
            ...
            if (BizValidate.ChkPeriodLocked(e.DBAccess, PAYT["period"].ToString()))
            {
        ...
        protected override void Document_Refresh_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            {
                foreach (DataRow drApd in apd.Rows)
                {
                    if (drApd.RowState != DataRowState.Deleted)
                    {
                        drApd["trandate"] = payt["trandate"];
                      //  drApd["invdate"] = payt["trandate"];
                    }
                }
            }
