	...
	public class Voucher_POR : BizRAD.BizApplication.VoucherBaseHelper
	{
            ...
            if (por1.Rows.Count > 0)
            {
                foreach (DataRow dr in por1.Rows)
                {
                    if (dr.RowState != DataRowState.Deleted)
                    {
                        ...
                        if (!BizFunctions.IsEmpty(dr["oriamt"]) && !BizFunctions.IsEmpty(dr["uomqty"]))
                        {
                            if (Convert.ToDecimal(dr["oriamt"])>0 && Convert.ToDecimal(dr["uomqty"])>0)
                            {
                                dr["baseunitprice"] = Convert.ToDecimal(dr["oriamt"]) / Convert.ToDecimal(dr["uomqty"]);
                            }
                        }
                    }
                }
        ...
        protected override void AddDetailF2Condition(object sender, DetailF2EventArgs e)
        {
            ...
            switch (e.MappingName)
            {
                case "matnum":
                    ...
                    e.DefaultCondition = "matnum in(select matnum from matm2 where apnum='"+porh["apnum"].ToString()+"')";
                    break;

                case "apmatnum":
                    e.Condition = BizFunctions.F2Condition("apmatnum", (sender as TextBox).Text);
                    e.DefaultCondition = "apmatnum in(select apmatnum from vAPMATM where apnum='" + porh["apnum"].ToString() + "')";
                    break;

                case "svccode":
                    e.Condition = BizFunctions.F2Condition("svccode", (sender as TextBox).Text);
        ...
        protected override void AddDetailF3Condition(object sender, DetailF3EventArgs e)
        {
            ...
            switch (e.MappingName)
            {
                case "matnum":
                    {
                        ...
                        e.CurrentRow["dpriceConvert"] = e.F2CurrentRow["dpriceConvert"];
                ...
                case "svccode":
                    {
                        e.CurrentRow["svcdesc"] = e.F2CurrentRow["svcdesc"];
                    }
                    break;
