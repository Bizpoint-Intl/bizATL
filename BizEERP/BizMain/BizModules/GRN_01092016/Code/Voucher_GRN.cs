...
namespace ATL.GRN
{
    public class Voucher_GRN : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        protected override void Document_SaveBegin_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            {
                if (grnh["type"].ToString().Trim() == "SITE")
                {     
                            ...
                            if (grnh["status"].ToString().Trim() == Common.DEFAULT_DOCUMENT_STATUSP)
                            {
                                ...
                                for (int i = 0; i <= 1; i++)
                                {
                                    ...
                                    if (i == 1)
                                    {
                                        ...
                                        dr_mwt["location"]  = "HQ";
                                        //dr_mwt["qty"] = -(decimal)dr["qty"];
                                        dr_mwt["qty"] = -(decimal)dr["qty"]; // Jason 20/01/2015
                ...
                {
                    DataTable GroupGldAcc = BizFunctions.ExecuteQuery(dbaccess.DataSet, "SELECT matnum,detail,pqty*uomqty qty,uom,'' barcode,price FROM GRN1 where refnum ='" + grnh["refnum"] + "' and ISNULL(sn,0)=0"
                ...
                case "grnh_ponum":
                    ...
                    if (!ponum.Equals(""))
                    {
                        ...
                        if (porhTmp.Rows.Count != 0)
                        {
                            ...
                            string selectPor1 = "SELECT ponum,matnum,uom,discamt,detail,sum(qty) as qty,price,sn,uom,uomcode,pqty,uomqty,memo,whnum from " +
                                "(SELECT refnum as ponum, matnum,uom,discamt,detail, pqty, qty,price,sn,uomcode,uomqty,memo,whnum from por1 UNION ALL" +
                                " SELECT ponum, matnum,uom,discamt,detail,pqty,-qty as qty,0 as price,sn,uomcode,uomqty,memo,whnum from grn1 WHERE refnum <>'" + grnh["refnum"].ToString().Trim() + "')a" +
                                "  WHERE ponum = '" + grnh["ponum"].ToString().Trim() + "' GROUP BY ponum,matnum,uom,discamt,detail,price,sn,uomcode,pqty,uomqty,memo,whnum" +
                                " HAVING SUM(qty) >0";
