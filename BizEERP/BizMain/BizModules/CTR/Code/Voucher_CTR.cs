...
namespace ATL.CTR
{
    public class Voucher_CTR : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        protected bool opened, isMouseClicked, enableDocSave, EnableMiscDocSave = false;
        string strFileName, flag;
        ...
        protected override void Document_Form_OnLoad(object sender, DocumentEventArgs e)
        {
            ...
            DataTable CTR20 = e.DBAccess.DataSet.Tables["CTR20"];
            ...
            EnableMiscDocSave = Convert.ToBoolean(System.Configuration.ConfigurationManager.AppSettings.Get("EnableMiscDocSave"));
        ...
        private void Initialise()
        {
            ...
            //GetEquipmentCost(); // Removed: 25/07/2014 - Jason
        ...
        Document_Save_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            DataRow ctrh = this.dbaccess.DataSet.Tables["ctrh"].Rows[0];
            DataTable ctr19 = this.dbaccess.DataSet.Tables["ctr19"];
            DataTable ctr20 = this.dbaccess.DataSet.Tables["ctr20"];

            


            foreach (DataRow dr19 in ctr19.Rows)
            {
                if (dr19.RowState != DataRowState.Deleted)
                {
                    if (BizFunctions.IsEmpty(dr19["projectid"]))
                    {
                        dr19["projectid"] = ctrh["projectid"];
                    }

                    if (BizFunctions.IsEmpty(dr19["sitenum"]))
                    {
                        dr19["sitenum"] = ctrh["sitenum"];
                    }

                    if (BizFunctions.IsEmpty(dr19["ccnum"]))
                    {
                        dr19["ccnum"] = "CLN";
                    }
                }
            ...
            string status = ctrh["status"].ToString();
            ...
            if (flag != "ADH")
            {
                if (!BizFunctions.IsEmpty(ctrh["totalmonths"]))
                {
                    ...
                    {
                        MessageBox.Show("Invalid Number of Months!. Only greater than 0 Monhts is valid", "ATL", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        e.Handle = false;
                    }
                    else
                    {
                        if (BizFunctions.IsEmpty(ctrh["generatebill"]))
                        {
                            ctrh["generatebill"] = 0;
                        }
                        if ((bool)ctrh["generatebill"])
                        {
                            int noOfMonths = Convert.ToInt32(ctrh["totalmonths"]);

                            if (ctr20.Rows.Count > 0)
                            {
                                BizFunctions.DeleteAllRows(ctr20);
                            }

                            for (int i = 0; i < noOfMonths; i++)
                            {
                                int number = i + 1;
                                DataRow insertCTR20 = ctr20.NewRow();
                                insertCTR20["billdate"] = TimeUtilites.TimeTools.GetLastDateMonth(Convert.ToDateTime(ctrh["commencedate"]).AddMonths(i));
                                insertCTR20["billingnumber"] = ctrh["refnum"].ToString() + "-" + number.ToString();
                                //insertCTR20["remark"] = "Billing for Month: " + Convert.ToDateTime(insertCTR20["billdate"]).Month.ToString() + " ";
                                ctr20.Rows.Add(insertCTR20);
                            }
                        }
                ...
                if(ctr20.Rows.Count>0)
                {
                    decimal totalbillingamt = SumUpBilling();
                    foreach (DataRow dr20 in ctr20.Rows)
                    {
                        if (dr20.RowState != DataRowState.Deleted)
                        {
                            if(BizFunctions.IsEmpty(dr20["billingamt"]))
                            {
                                dr20["billingamt"] = totalbillingamt;
                            }
                            else if (Convert.ToDecimal(dr20["billingamt"]) == 0)
                            {
                                dr20["billingamt"] = totalbillingamt;
                            }
                        }
                ...
                if (ctr20.Rows.Count > 0)
                {
                    decimal totalbillingamt = 0;
                    foreach (DataRow dr20 in ctr20.Rows)
                    {
                        if (dr20.RowState != DataRowState.Deleted)
                        {
                            if (!BizFunctions.IsEmpty(dr20["invnum"]))
                            {
                                if (BizFunctions.IsEmpty(dr20["actualbillingamt"]))
                                {
                                    dr20["actualbillingamt"] = 0;
                                }

                                totalbillingamt = totalbillingamt + Convert.ToDecimal(dr20["actualbillingamt"]);
                                
                            }                           
                        }
                    }

                    ctrh["balbudget"] = totalbillingamt;
            ...
            decimal AllTotalBillAmt = 0;
            if (BizFunctions.IsEmpty(ctrh["ttlbillingamt"]))
            {
                foreach (DataRow dr20 in ctr20.Rows)
                {
                    if (dr20.RowState != DataRowState.Deleted)
                    {
                        AllTotalBillAmt = AllTotalBillAmt + Convert.ToDecimal(dr20["billingamt"]);
                    }
                }
                ctrh["ttlbillingamt"] = AllTotalBillAmt;

            }
            else if (Convert.ToDecimal(ctrh["ttlbillingamt"]) == 0)
            {
                foreach (DataRow dr20 in ctr20.Rows)
                {
                    if (dr20.RowState != DataRowState.Deleted)
        ...
        private decimal SumUpBilling()
        {
            decimal total = 0;

            DataTable ctr19 = this.dbaccess.DataSet.Tables["ctr19"];

            if (ctr19.Rows.Count > 0)
            {
                foreach (DataRow dr19 in ctr19.Rows)
                {
                    if (dr19.RowState != DataRowState.Deleted)
                    {
                        if (BizFunctions.IsEmpty(dr19["oriamt"]))
                        {
                            dr19["oriamt"] = 0;
                        }
                        total = total + Convert.ToDecimal(dr19["oriamt"]);
                    }
                }
            }
        ...
        protected override void AddDocumentF2Condition(object sender, DocumentF2EventArgs e)
        {
            ...
            switch (e.ControlName)
            {
                case "ctrh_arnum":
                    ...
                    {
                        e.DefaultCondition = "(arm.arnum like '%" + ctrh["arnum"].ToString().Trim() + "%' OR arm.arname like '" + ctrh["arnum"].ToString().Trim() + "%') and arm.[status]<>'V' and isnull(iscoach,0)=0";
             //           e.Condition = BizFunctions.F2Condition("arnum,arname", (sender as TextBox).Text);
                ...
                case "ctrh_sitenum":
                    if (!BizFunctions.IsEmpty(ctrh["arnum"]))
                    {
                       // e.Condition = BizFunctions.F2Condition("sitenum,sitename", (sender as TextBox).Text);
                        if (!BizFunctions.IsEmpty(ctrh["sitenum"]))
                        {
                            if (ctrh["sitenum"].ToString().Trim() != string.Empty)
                            {
                                e.DefaultCondition = "(sitm.sitenum like '" + ctrh["sitenum"].ToString() + "%' OR sitm.sitename like '" + ctrh["sitenum"].ToString() + "%') and sitm.[status]<>'V' and isnull(iscoach,0)=0";
                            }
                        }
                    }
                    else
        ...
        protected override void AddDocumentF3Condition(object sender, DocumentF3EventArgs e)
        {
            ...
            switch (e.ControlName)
            {
                ...
                case "ctrh_arnum":
                    {
                        ...
                        //ctrh["ptc"] = e.F2CurrentRow["ptc"];
                        //ctrh["email"] = e.F2CurrentRow["bemail"];
                        //ctrh["phone"] = e.F2CurrentRow["phone"];
                        //ctrh["hp"] = e.F2CurrentRow["hp"];
                        //ctrh["fax"] = e.F2CurrentRow["fax"];
                        //ctrh["parnum"] = e.F2CurrentRow["parnum"];

                        ctrh["arname"] = e.F2CurrentRow["arname"];
                        ctrh["billadd1"] = e.F2CurrentRow["addr1"];
                        ctrh["billadd2"] = e.F2CurrentRow["addr2"];
                        ctrh["billadd3"] = e.F2CurrentRow["addr3"];
                        ctrh["billadd4"] = e.F2CurrentRow["addr4"];
                        ctrh["ptc"] = e.F2CurrentRow["ptc"];
                        ctrh["email"] = e.F2CurrentRow["email"];
                        ctrh["phone"] = e.F2CurrentRow["phone"];
                        ctrh["hp"] = e.F2CurrentRow["hp"];
                        ctrh["fax"] = e.F2CurrentRow["fax"];
                        ctrh["parnum"] = e.F2CurrentRow["parnum"];
                ...
                case "ctrh_empnum":
                    {
                        ctrh["empname"] = e.F2CurrentRow["empname"];
                    }
                    break;
        ...
        protected override void Document_SaveBegin_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            if (BizFunctions.IsEmpty(ctrh["docunum"]))
            {
                ctrh["docunum"] = ctrh["refnum"];
            }
            ...
            if (EnableMiscDocSave)
            {
                if (ctr10.Rows.Count > 0)
                {
                    try
                    {
                        ...
                        {
                            MessageBox.Show("Upload of Docs unsuccessful!. Please try again.", "ATL", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        protected override void Document_Refresh_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            //if (BizFunctions.IsEmpty(csoh["terms"]))
            //{
            //    csoh["terms"] = "The above is subject to prevailing GST rate";
            //}
            ...
            foreach (DataRow dr20 in ctr20.Rows)
            {
                if (dr20.RowState != DataRowState.Deleted)
                {
                    dr20["invnum"] = GetBillingInvoice(dr20["billingnumber"].ToString());
                    dr20["actualbillingamt"] = GetBillingInvoiceAmt(dr20["billingnumber"].ToString());
                }
            }
        ...
        #region Jason - To Set Auto Calculate and use in various events
        private void AutoCalc()
        {
            DataRow ctrh = dbaccess.DataSet.Tables["ctrh"].Rows[0];
            DataTable ctr19 = dbaccess.DataSet.Tables["ctr19"];
            ...
            if (ctr19.Rows.Count > 0)
            {
                foreach (DataRow dr in ctr19.Rows)
                {
                    if (dr.RowState != DataRowState.Deleted)
                    {
                        BizFunctions.UpdateDataRow(ctrh, dr, "refnum/arnum/docunum/flag/status/user/modified");

                        if (BizFunctions.IsEmpty(dr["qty"]))
                        {
                            dr["qty"] = 0;
                        }
                        ...
                        {
                            if (BizFunctions.IsEmpty(dr["grosamt"]))
                            {
                                dr["grosamt"] = 0;
                            }
                            ...
                            if (BizFunctions.IsEmpty(dr["discamt"]))
                            {
                                dr["discamt"] = 0;
                            }
                        ...
                        //ctr1_grosamt += (decimal)dr["grosamt"];
                        //ctr1_discamt += (decimal)dr["discamt"];
                        //ctr1_oriamt += (decimal)dr["oriamt"];
                    }
                }
            }

            //ctrh["grosamt"] = ctr1_grosamt;
            //ctrh["discamt"] = ctr1_discamt;
            //ctrh["oriamt"] = ctr1_oriamt;
        ...
        protected override void Document_Print_OnClick(object sender, DocumentPrintEventArgs e)
        {
            ...
            switch (e.ReportName)
            {
                ...
                case "Contract Form 1":
                    string getContractReport1 = "Select " +
                                                    "refnum, " +
                                                    "arnum, " +
                                                    "arname, " +
                                                    "CostDesc, " +
                                                    "SUM(CostAmount) AS CostAmount, " +
                                                    "CASE WHEN SUM(Convert(decimal(16,2),CostAmount))>0.00 AND TOTALAMT > 0 THEN (SUM(Convert(decimal(16,2),CostAmount))/TOTALAMT)*100 ELSE 0 END AS PercentOfCost " +
                                                "FROM " +
                                                "( " +
                                                    "SELECT  " +
                                                        "B.refnum, B.arnum, B.arname, " +
                                                        "SUM(Convert(decimal(16,2),B.MANPOWERCOST)) AS Manpower,  " +
                                                        "SUM(Convert(decimal(16,2),B.MATERIALCOST)) AS Material,  " +
                                                        "SUM(Convert(decimal(16,2),B.MACHINERYCOST)) AS Machinery,  " +
                                                        "SUM(Convert(decimal(16,2),B.TOILETRIESCOST)) AS Toiletry,  " +
                                                        "SUM(Convert(decimal(16,2),B.CHEMICALCOST)) AS Chemical,  " +
                                                        "SUM(Convert(decimal(16,2),B.MANPOWERCOST))+SUM(Convert(decimal(16,2),B.MATERIALCOST))+SUM(Convert(decimal(16,2),B.MACHINERYCOST))+SUM(Convert(decimal(16,2),B.TOILETRIESCOST))+SUM(Convert(decimal(16,2),B.CHEMICALCOST)) AS TOTALAMT  " +
                                                    "FROM  " +
                                                    "(  " +
                                                        "SELECT  " +
                                                                "A.refnum,  " +
                                                                ...
                                                                "A.TOILETRIESCOST,  " +
                                                                "A.CHEMICALCOST  " +
                        //"A.MANPOWERCOST+A.MATERIALCOST+A.MACHINERYCOST+A.TOILETRIESCOST+A.CHEMICALCOST AS TOTALAMT  " +
                                                                    "FROM (  " +
                                                                            "SELECT refnum, ISNULL(oriamt,0.00) as manpowercost, 0.00 as equipmentcost, 0.00 as materialcost, 0.00 as machinerycost, 0.00 as toiletriescost, 0.00 as chemicalcost FROM CTR1 where refnum='" + ctrh["refnum"].ToString() + "'  " +
                                                                            "UNION ALL  " +
                                                                            "SELECT refnum, 0.00 as manpowercost, ISNULL(price,0.00) as equipmentcost, 0.00 as materialcost, 0.00 as machinerycost, 0.00 as toiletriescost, 0.00 as chemicalcost  FROM CTR3 where refnum='" + ctrh["refnum"].ToString() + "'  " +
                                                                            "UNION ALL  " +
                                                                            "SELECT refnum, 0.00 as manpowercost, 0.00 as equipmentcost, ISNULL(price,0.00) as materialcost, 0.00 as machinerycost, 0.00 as toiletriescost, 0.00 as chemicalcost  FROM CTR5 where refnum='" + ctrh["refnum"].ToString() + "'  " +
                                                                            "UNION ALL  " +
                                                                            "SELECT refnum,0.00 as manpowercost, 0.00 as equipmentcost, 0.00 as materialcost, ISNULL(price,0.00) as machinerycos, 0.00 as toiletriescost, 0.00 as chemicalcost  FROM CTR7 where refnum='" + ctrh["refnum"].ToString() + "'  " +
                                                                            "UNION ALL  " +
                                                                            "SELECT refnum,0.00 as manpowercost, 0.00 as equipmentcost, 0.00 as materialcost, 0.00 as machinerycost, ISNULL(price,0.00) as toiletriescost, 0.00 as chemicalcost  FROM CTR8 where refnum='" + ctrh["refnum"].ToString() + "'  " +
                                                                            "UNION ALL  " +
                                                                            "SELECT refnum,0.00 as manpowercost, 0.00 as equipmentcost, 0.00 as materialcost, 0.00 as machinerycost, 0.00 as toiletriescost, ISNULL(price,0.00) as chemicalcost  FROM CTR9 where refnum='" + ctrh["refnum"].ToString() + "'  " +
                                                                        ")A  " +
                                                                    "LEFT JOIN CTRH CH ON A.refnum=CH.refnum  " +
                                                                    "GROUP BY A.refnum, CH.ARNUM, CH.ARNAME,A.chemicalcost,A.equipmentcost,A.machinerycost,A.manpowercost,A.materialcost,A.toiletriescost  " +
                                                    ")B  " +
                                                    "GROUP BY B.refnum, B.ARNUM, B.arname " +
                                                ") X " +
                                                "UNPIVOT " +
                                                "( " +
                                                    "CostAmount FOR CostDesc IN (Manpower,Material,Machinery,Chemical,Toiletry) " +
                                                ") AS Y " +
                                                "GROUP BY refnum,arnum,arname,CostDesc,TOTALAMT ";
                    //e.DataSource = ctrds1(); 
                    selectedCollection.Add("getContractReport1", getContractReport1);
                ...
                case "Contract Form 2":
                    string getContractReport2 = "SELECT " +
                                                        "B.refnum, " +
                                                        "B.arnum, " +
                                                        "B.arname, " +
                                                        "SUM(Convert(decimal(16,2),B.MANPOWERCOST)) AS MANPOWERCOST, " +
                                                        "SUM(Convert(decimal(16,2),B.[MANPOWER%])) AS [MANPOWER%], " +
                                                        "SUM(Convert(decimal(16,2),B.MATERIALCOST)) AS MATERIALCOST, " +
                                                        "SUM(Convert(decimal(16,2),B.[MATERIAL%])) AS [MATERIAL%], " +
                                                        "SUM(Convert(decimal(16,2),B.MACHINERYCOST)) AS MACHINERYCOST, " +
                                                        "SUM(Convert(decimal(16,2),B.[MACHINERY%])) AS [MACHINERY%], " +
                                                        "SUM(Convert(decimal(16,2),B.TOILETRIESCOST)) AS TOILETRIESCOST, " +
                                                        "SUM(Convert(decimal(16,2),B.[TOILETRY%])) AS [TOILETRY%], " +
                                                        "SUM(Convert(decimal(16,2),B.CHEMICALCOST)) AS CHEMICALCOST, " +
                                                        "SUM(Convert(decimal(16,2),B.[CHEMICAL%])) AS [CHEMICAL%], " +
                                                        "B.TOTALAMT " +
                                                    "FROM " +
                                                    "( " +
                                                            "SELECT " +
                                                            ...
                                                            "CH.ARNUM, CH.ARNAME, " +
                                                                "A.MANPOWERCOST, " +
                                                                "CASE WHEN SUM(Convert(decimal(16,2),A.manpowercost))>0.00 AND CH.ttlbillingamt > 0  THEN (SUM(Convert(decimal(16,2),A.manpowercost))/CH.ttlbillingamt)*100 ELSE 0 END AS [MANPOWER%], " +
                                                                "A.MATERIALCOST, " +
                                                                "CASE WHEN SUM(Convert(decimal(16,2),A.materialcost))>0.00 AND CH.ttlbillingamt > 0  THEN (SUM(Convert(decimal(16,2),A.materialcost))/CH.ttlbillingamt)*100 ELSE 0 END AS [MATERIAL%], " +
                                                                "A.MACHINERYCOST, " +
                                                                "CASE WHEN SUM(Convert(decimal(16,2),A.machinerycost))>0.00 AND CH.ttlbillingamt > 0  THEN (SUM(Convert(decimal(16,2),A.machinerycost))/CH.ttlbillingamt)*100 ELSE 0 END AS [MACHINERY%], " +
                                                                "A.TOILETRIESCOST, " +
                                                                "CASE WHEN SUM(Convert(decimal(16,2),A.toiletriescost))>0.00 AND CH.ttlbillingamt > 0 THEN (SUM(Convert(decimal(16,2),A.toiletriescost))/CH.ttlbillingamt)*100 ELSE 0 END AS [TOILETRY%], " +
                                                                "A.CHEMICALCOST, " +
                                                                "CASE WHEN SUM(Convert(decimal(16,2),A.chemicalcost))>0.00 AND CH.ttlbillingamt > 0 THEN (SUM(Convert(decimal(16,2),A.chemicalcost))/CH.ttlbillingamt)*100 ELSE 0 END AS [CHEMICAL%], " +
                                                                "CH.ttlbillingamt AS TOTALAMT " +
                                                            "FROM " +
                                                            "( " +
                                                            "SELECT refnum, ISNULL(oriamt,0.00) as manpowercost, 0.00 as equipmentcost, 0.00 as materialcost, 0.00 as machinerycost, 0.00 as toiletriescost, 0.00 as chemicalcost FROM CTR1 where refnum='" + ctrh["refnum"].ToString() + "' " +
                                                            "UNION ALL " +
                                                            "SELECT refnum, 0.00 as manpowercost, ISNULL(oriamt,0.00) as equipmentcost, 0.00 as materialcost, 0.00 as machinerycost, 0.00 as toiletriescost, 0.00 as chemicalcost  FROM CTR3 where refnum='" + ctrh["refnum"].ToString() + "' " +
                                                            "UNION ALL " +
                                                            "SELECT refnum, 0.00 as manpowercost, 0.00 as equipmentcost, ISNULL(oriamt,0.00) as materialcost, 0.00 as machinerycost, 0.00 as toiletriescost, 0.00 as chemicalcost  FROM CTR5 where refnum='" + ctrh["refnum"].ToString() + "' " +
                                                            "UNION ALL " +
                                                            "SELECT refnum,0.00 as manpowercost, 0.00 as equipmentcost, 0.00 as materialcost, ISNULL(oriamt,0.00) as machinerycos, 0.00 as toiletriescost, 0.00 as chemicalcost  FROM CTR7 where refnum='" + ctrh["refnum"].ToString() + "' " +
                                                            "UNION ALL " +
                                                            "SELECT refnum,0.00 as manpowercost, 0.00 as equipmentcost, 0.00 as materialcost, 0.00 as machinerycost, ISNULL(oriamt,0.00) as toiletriescost, 0.00 as chemicalcost  FROM CTR8 where refnum='" + ctrh["refnum"].ToString() + "' " +
                                                            "UNION ALL " +
                                                            "SELECT refnum,0.00 as manpowercost, 0.00 as equipmentcost, 0.00 as materialcost, 0.00 as machinerycost, 0.00 as toiletriescost, ISNULL(oriamt,0.00) as chemicalcost  FROM CTR8 where refnum='" + ctrh["refnum"].ToString() + "' " +
                                                            ")A " +
                                                            "LEFT JOIN CTRH CH ON A.refnum=CH.refnum " +
                                                            "GROUP BY A.refnum, CH.ARNUM, CH.ARNAME,CH.ttlbillingamt,A.chemicalcost,A.equipmentcost,A.machinerycost,A.manpowercost,A.materialcost,A.toiletriescost " +
                                                        ")B " +
                                                        "GROUP BY B.refnum, B.ARNUM, B.arname,B.TOTALAMT ";

                    selectedCollection.Add("getContractReport2", getContractReport2);
                ...
                case "Contract Form 3":
                    string getContractReport3 = "SELECT " +
                                                        "B.refnum, " +
                                                        "B.arnum, " +
                                                        "B.arname, " +
                                                        "SUM(Convert(decimal(16,2),B.MANPOWERCOST)) AS MANPOWERCOST, " +
                    ...
                    selectedCollection.Add("getContractReport3", getContractReport3);
        ...
        private void Voucher_CTRH_ColumnChanged(object sender, DataColumnChangeEventArgs e)
        {
            ...
            switch (e.Column.ColumnName)
            {
                ...
                case "arnum":
                    {
                        //if (!BizFunctions.IsEmpty(ctrh["arnum"].ToString()))
                        //{
                        //    if (BizFunctions.IsEmpty(ctrh["sitenum"]))
                        //    {
                        //        string GetSitenumI = "Select top 1 * from sitm where arnum='" + ctrh["arnum"].ToString() + "'";
                        //        this.dbaccess.ReadSQL("SITM", GetSitenumI);
                        //        DataTable sitm = this.dbaccess.DataSet.Tables["SITM"];
                        //        if (sitm.Rows.Count > 0)
                        //        {
                ...
                case "empnum":
                    {
                        if (!BizFunctions.IsEmpty(ctrh["empnum"]))
                        {
                            DataRow dr = BizLogicTools.Tools.GetCommonEmpDataRow(ctrh["empnum"].ToString());
                            ctrh["empname"] = dr["empname"].ToString();
                        }
                    }
                    break;
            }
        ...
        private string GetBillingInvoice(string billingno)
        {
            string invnum = "";

            string str = "SELECT top 1 refnum FROM SIVH where custpo='"+billingno+"' and [status]='P'";

            this.dbaccess.ReadSQL("TempBillRef", str);

            DataTable dt = this.dbaccess.DataSet.Tables["TempBillRef"];

            if (dt.Rows.Count > 0)
            {
                invnum = dt.Rows[0]["refnum"].ToString().Trim();
            }

            return invnum;
        }

        private decimal GetBillingInvoiceAmt(string billingno)
        {
            decimal postamt = 0;

            string str = "SELECT top 1 postamt FROM SIVH where custpo='" + billingno + "' and [status]='P'";

            this.dbaccess.ReadSQL("TempBillRefAmt", str);

            DataTable dt = this.dbaccess.DataSet.Tables["TempBillRefAmt"];

            if (dt.Rows.Count > 0)
            {
                if (BizFunctions.IsEmpty(dt.Rows[0]["postamt"]))
                {
                    dt.Rows[0]["postamt"] = 0;
                }
                postamt = Convert.ToDecimal(dt.Rows[0]["postamt"]);
            }
