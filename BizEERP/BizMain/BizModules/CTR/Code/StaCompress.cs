...
{
    public partial class StaCompress : Form
    {
        ...
        public StaCompress(DBAccess dbaccess,string TnameHeader, string Tname, string Ref)
        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                DataRow table = dbaccess.DataSet.Tables[headerTable].Rows[0];
                DataTable Table = dbaccess.DataSet.Tables[TableName];
                string sZipFileName = table[refnum].ToString() + "-" + BizLogicTools.Tools.getGUID();
                ...
                try
                {
                    ...
                    using (ZipOutputStream s = new ZipOutputStream(File.Create(temploc + "\\" + sZipFileName + ".zip")))
                    {
                        ...
                        DataRow dr_Table = Table.NewRow();
                        dr_Table["trackingno"] = sZipFileName;
                        dr_Table["filename"] = sZipFileName + ".zip";
                        dr_Table["remark"] = txtRemarkCom.Text;
                        dr_Table["templocation"] = temploc + sZipFileName + ".zip";
                        dr_Table["flname"] = cb_type.Text;
                        Table.Rows.Add(dr_Table);
                        s.Finish();
                        s.Close();
                    }
        ...
        private void GetFolderType()
        {

            string sql1 = "Select fldmname,subFolder,fileNmFmt from FLDM where [status]<>'V'";
            this.dbaccess.ReadSQL("FLDM", sql1);

            DataTable FLDM = this.dbaccess.DataSet.Tables["FLDM"];

            if (FLDM.Rows.Count > 0)
            {

                DataRow drInsertType = this.dbaccess.DataSet.Tables["FLDM"].NewRow();

                drInsertType["fldmname"] = "NIL";

                this.dbaccess.DataSet.Tables["FLDM"].Rows.Add(drInsertType);

                cb_type.DataSource = this.dbaccess.DataSet.Tables["FLDM"];
                cb_type.DisplayMember = this.dbaccess.DataSet.Tables["FLDM"].Columns["fldmname"].ColumnName.ToString();
                cb_type.ValueMember = this.dbaccess.DataSet.Tables["FLDM"].Columns["fldmname"].ColumnName.ToString();
            }
        }
