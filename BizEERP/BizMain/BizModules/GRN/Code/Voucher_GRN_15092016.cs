                    ...
                    foreach (DataRow dr in grn1.Rows)
                    {
                        if (dr.RowState != DataRowState.Deleted)
                        {
                            ...
                            if (grnh["status"].ToString().Trim() == Common.DEFAULT_DOCUMENT_STATUSP)
                            {
                                ...
                                dr_mwt1["location"] = dr["whnum"]; // Jason : 09/02/2015: Used this for extraction as whnum is already set in the POR1
                                dr_mwt1["qty"] = (decimal)dr["pqty"];
                                dr_mwt1["stdcost"] = dr["price"];// wy 20/08/2016
                                dr_mwt1["whnum"] = dr["whnum"];
                    ...
                    if (!ponum.Equals(""))
                    {
                        ...
                        if (porhTmp.Rows.Count != 0)
                        {
                            ...
                            string selectPor1 = "SELECT ponum,matnum,uom,discamt,detail,sum(qty) as qty,price,uprice,sn,uom,uomcode,pqty,uomqty,memo,whnum from " +
                                "(SELECT refnum as ponum, matnum,uom,discamt,detail, pqty, qty,price,price/qty uprice,sn,uomcode,uomqty,memo,whnum from por1 UNION ALL" +
                                " SELECT ponum, matnum,uom,discamt,detail,pqty,-qty as qty,0 as price,uprice,sn,uomcode,uomqty,memo,whnum from grn1 WHERE refnum <>'" + grnh["refnum"].ToString().Trim() + "')a" +
