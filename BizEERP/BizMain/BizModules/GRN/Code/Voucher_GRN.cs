...
namespace ATL.GRN
{
    public class Voucher_GRN : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        protected override void Document_Save_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            if (BizFunctions.IsEmpty(grnh["ccnum"]))
            {
                grnh["ccnum"] = "CLN";
            }
            foreach (DataRow dr1 in grn1.Rows)
            {
                if (dr1.RowState != DataRowState.Deleted)
                {
                    ...
                    {
                        dr1["ccnum"] = grnh["ccnum"];
                    }
                    if (BizFunctions.IsEmpty(dr1["projectid"]))
                    {
                        if (!BizFunctions.IsEmpty(grnh["projectid"]))
                        {
                            dr1["projectid"] = grnh["projectid"];
                        }
                    }
                }
            }
            ...
            foreach (DataRow dr in grn1.Rows)
            {
                if (dr.RowState != DataRowState.Deleted)
                {
                    ...
                    if (!BizFunctions.IsEmpty(dr["pqty"]))
                    {
                        if (!BizFunctions.IsEmpty(dr["uomqty"]))
                        {
                            if (Convert.ToDecimal(dr["pqty"]) > 0)
                            {
                                dr["qty"] = Convert.ToDecimal(dr["pqty"]) * Convert.ToDecimal(dr["uomqty"]);
                            }
                        }
                    }
                    //   case "pqty":
            ...
            if (grn1.Rows.Count > 0)
            {
                foreach (DataRow dr1 in grn1.Rows)
                {
                    if (dr1.RowState != DataRowState.Deleted)
                    {
                        if (!BizFunctions.IsEmpty(grnh["whnum"]) && BizFunctions.IsEmpty(dr1["whnum"]))
                        {
                            dr1["whnum"] = grnh["whnum"];
                        }
                    }
                }
            }
        ...
        protected override void Document_Reopen_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            if (grnh["flag"].ToString().Trim() != "GRN")
            {
                e.Handle = false;
            }
            else if (BizValidate.ChkPeriodLocked(e.DBAccess, grnh["period"].ToString()))
            {
            ...
            {
                e.DBAccess.RemoteStandardSQL.ExecuteNonQuery("DELETE FROM mwt" + Common.DEFAULT_SYSTEM_YEAR + " WHERE refnum='" + grnh["refnum"].ToString().Trim() + "'");
                //e.DBAccess.RemoteStandardSQL.ExecuteNonQuery("DELETE FROM GLD" + Common.DEFAULT_SYSTEM_YEAR + " WHERE refnum='" + grnh["refnum"].ToString().Trim() + "'");
            }
        ...
        protected override void Document_SaveBegin_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            //grnh["type"] = "LOCAL";
            //#endregion
            if (BizFunctions.IsEmpty(grnh["type"]))
            {
                grnh["type"] = "LOCAL";
            }
            ...
            foreach (DataRow dr in grn1.Rows)
            {
                if (dr.RowState != DataRowState.Deleted)
                {
                    BizFunctions.UpdateDataRow(grnh, dr, "status");
                }
            }
            ...
            {
                if (grnh["type"].ToString().Trim().ToLower() == "site")
                {     
                    ...
                    foreach (DataRow dr in grn1.Rows)
                    {
                        if (dr.RowState != DataRowState.Deleted)
                        {
                            BizFunctions.UpdateDataRow(grnh, dr, "refnum,trandate,status,flag,user,created,modified");
                            ...
                            if (grnh["status"].ToString().Trim() == Common.DEFAULT_DOCUMENT_STATUSP)
                            {
                                #region assigning rows to local mwt +HQ / -HQ
                                for (int i = 0; i <= 1; i++)
                                {
                                    if (i == 0)
                                    {
                                        ...
                                        dr_mwt["barcode"] = dr["sn"];
                                        dr_mwt["matnum"] = dr["matnum"];
                                        dr_mwt["detail"] = dr["detail"];
                                        //dr_mwt["uom"] = dr["uomcode"]; // Jason 20/01/2015
                                        dr_mwt["uom"] = dr["uom"];
                                        dr_mwt["docunum"] = dr["refnum"];
                                        //dr_mwt["location"] = dr["tositenum"];
                                        dr_mwt["location"] = "HQ";
                                        dr_mwt["stdcost"] = dr["price"]; // wy 20/08/2016
                                        dr_mwt["qty"] = (decimal)dr["pqty"];
                                        dr_mwt["whnum"] = "MWH";//increase qty in main warehouse,here is the fromsitenum(local) if is outlets
                                        dr_mwt["guid"] = Tools.getGUID();
                                        dr_mwt["trandate"] = dr["trandate"];
                                        dr_mwt["year"] = grnh["systemyear"];
                                        ...
                                        dr_mwt["remarks"] = dr["remark"].ToString().Trim();
                                        dr_mwt["flag"] = grnh["flag"];
                                        mwt.Rows.Add(dr_mwt);
                                    }
                                    if (i == 1)
                                    {
                                        ...
                                        dr_mwt["detail"] = dr["detail"];
                                        dr_mwt["stdcost"] = dr["price"];// wy 20/08/2016
                                        dr_mwt["uom"] = dr["uom"]; // Jason 20/01/2015
                                        dr_mwt["docunum"] = dr["refnum"];
                                        //dr_mwt["location"] = dr["fromsitenum"];
                                        dr_mwt["location"]  = "HQ";
                                        //dr_mwt["qty"] = -(decimal)dr["qty"];
                                        dr_mwt["qty"] = -(decimal)dr["pqty"]; // Jason 20/01/2015
                                        dr_mwt["whnum"] = "MWH";//Decrease qty in Stock In Transit warehouse(Location HQ)
                                        dr_mwt["guid"] = Tools.getGUID();
                                        dr_mwt["trandate"] = dr["trandate"];
                                        dr_mwt["year"] = grnh["systemyear"];
                                        dr_mwt["created"] = dr["created"];
                                ...
                                #region assigning rows to mwt - target site
                                DataRow dr_mwt1 = mwt.NewRow();
                                dr_mwt1["refnum"] = dr["refnum"];
                                dr_mwt1["barcode"] = dr["sn"];
                                dr_mwt1["matnum"] = dr["matnum"];
                                dr_mwt1["detail"] = dr["detail"];
                                dr_mwt1["uom"] = dr["uom"];
                                dr_mwt1["docunum"] = dr["refnum"];
                                //dr_mwt1["location"] = dr["tositenum"];
                                //dr_mwt1["location"] = BizLogicTools.Tools.GetSitenum(grnh["whnum"].ToString(), this.dbaccess); // Jason : 09/02/2015: Commented to enable users to GRN multiple locations
                                dr_mwt1["location"] = grnh["whnum"]; // Jason : 09/02/2015: Used this for extraction as whnum is already set in the POR1
                                dr_mwt1["qty"] = (decimal)dr["pqty"];
                                dr_mwt1["stdcost"] = dr["price"];// wy 20/08/2016
                                dr_mwt1["whnum"] = grnh["whnum"];
                                dr_mwt1["guid"] = Tools.getGUID();
                                dr_mwt1["trandate"] = dr["trandate"];
                                dr_mwt1["year"] = grnh["systemyear"];
                                dr_mwt1["created"] = dr["created"];
                                dr_mwt1["modified"] = dr["modified"];
                                dr_mwt1["status"] = dr["status"].ToString().Trim();
                                dr_mwt1["user"] = dr["user"].ToString().Trim();
                                dr_mwt1["remarks"] = dr["remark"].ToString().Trim();
                                dr_mwt1["flag"] = grnh["flag"];
                                mwt.Rows.Add(dr_mwt1);
                ...
                {
                    DataTable GroupGldAcc = BizFunctions.ExecuteQuery(dbaccess.DataSet, "SELECT matnum,detail,pqty  qty,uom,'' barcode,price FROM GRN1 where refnum ='" + grnh["refnum"] + "' and ISNULL(sn,0)=0"
                                                                                        + " union all SELECT matnum,detail,1 qty,uom,barcode,isnull(0,price)price FROM GRN2 where refnum ='" + grnh["refnum"] + "'");
                ...
                {
                    string updatePORH = "Update PORH set grnum='" + grnh["refnum"].ToString() + "' where refnum='" + grnh["ponum"].ToString() + "'";
                    dbaccess.RemoteStandardSQL.ExecuteNonQuery(updatePORH);
                }
            }
            #endregion
        }
        ...
        protected override void AddDocumentF2Condition(object sender, DocumentF2EventArgs e)
        {
            ...
            {
                case "grnh_ponum":
                    e.DefaultCondition = " porh.apnum='" + grnh["apnum"].ToString() + "' ";
                    break;

                case "grnh_projectid":
                    if (!BizFunctions.IsEmpty(grnh["whnum"]))
                    {
                        e.DefaultCondition = " Projectid in (Select projectid from CTRH where sitenum in (Select top 1 sitenum from whm where whnum='" + e.CurrentRow["whnum"].ToString().Trim() + "') and [status]<>'V') ";
                    }
                    break;
            }
        }
                ...
                case "grnh_ponum":
                    ...
                    if (!ponum.Equals(""))
                    {
                        ...
                        if (porhTmp.Select().Length > 0)
                        {
                            ...
                            grnh["whnum"] = BizLogicTools.Tools.GetWhnum(BizLogicTools.Tools.GetSitenumFromTrq(porhTmp.Rows[0]["ponum"].ToString(), this.dbaccess), this.dbaccess);
                        ...
                        if (porhTmp.Rows.Count != 0)
                        {
                            ...
                            string selectPor1 = "SELECT ponum,matnum,uom,discamt,detail,sum(qty) as qty,price,uprice,sn,uom,uomcode,pqty,uomqty,memo,whnum from " +
                                "(SELECT refnum as ponum, matnum,uom,discamt,detail, pqty, qty,price,grosamt/qty uprice,sn,uomcode,uomqty,memo,whnum from por1 UNION ALL" +
                                " SELECT ponum, matnum,uom,discamt,detail,pqty,-qty as qty, price,uprice,sn,uomcode,uomqty,memo,whnum from grn1 WHERE refnum <>'" + grnh["refnum"].ToString().Trim() + "')a" +
                                "  WHERE ponum = '" + grnh["ponum"].ToString().Trim() + "' GROUP BY ponum,matnum,uom,discamt,detail,price,uprice,sn,uomcode,pqty,uomqty,memo,whnum" +
                                " HAVING SUM(qty) >0";
                        ...
                        if (porhTmp.Rows.Count != 0)
                        {
                            #region Steph - Import por2 to grn2
                            //Yushu Modified20100329-Do not sum price because there is special scenarios in ATL where there will be same items of different pricing
                        ...
                        e.CurrentRow["type"] = e.F2CurrentRow["type"];
                        e.CurrentRow["type2"] = e.F2CurrentRow["type2"];
                    }
                    AutoCalc();
                    break;               
            }
        ...
        private void btn_Extract_Click(object sender, EventArgs e)
        {
            ...
            if (grnh["apnum"].ToString() != string.Empty)
            {
                ...
                try
                {
                    ...
                    ExtractPOR ExtractPOR = new ExtractPOR(this.dbaccess, oriTable);
                    ExtractPOR.ShowDialog();
                    ExtractPOR.Focus();
        ...
        protected override void Document_Refresh_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            if (grn1.Rows.Count > 0)
            {
                foreach (DataRow dr1 in grn1.Rows)
                {
                    if (dr1.RowState != DataRowState.Deleted)
                    {
                        if (BizFunctions.IsEmpty(dr1["uomcode"]) && !BizFunctions.IsEmpty(dr1["matnum"]))
                        {
                            dr1["uomcode"] = BizLogicTools.Tools.GetMatnumPUOM(dr1["matnum"].ToString(), this.dbaccess);
                        }
                    }
                }
            }
            ...
            foreach (DataRow dr in grn1.Rows)
            {
                if (dr.RowState != DataRowState.Deleted)
                {
                    if (Convert.ToDecimal(dr["qty"]) > 0)
                    {
                        ...
                        dr["stdcost"] = (decimal)dr["oriamt"] / (decimal)dr["pqty"] * (decimal)grnh["exrate"];

                        grn1_grosamt += (decimal)dr["grosamt"];
                        grn1_discamt += (decimal)dr["discamt"];
                        grn1_oriamt += (decimal)dr["oriamt"];
                        if (dr["sn"].ToString() == "True") { insertGRN2(dr["matnum"].ToString(), dr["pqty"].ToString(), dr["detail"].ToString(), dr["uom"].ToString()); }
                    }
                }
            }
