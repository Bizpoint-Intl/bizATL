...
namespace ASOMS.CTR
{
    public class Voucher_CTR : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        private void Refresh_Header()
        {
            ...
            ctrh["totalmonths"] = ASOMS.TimeUtilites.TimeTools.MonthDiff(Convert.ToDateTime(ctrh["commencedate"]), Convert.ToDateTime(ctrh["enddate"]));
            RefreshMonthNo();
            headerFlag = false;
        }
        ...
        private void Refresh_Budget()
        {
            ...
            if (BizFunctions.IsEmpty(ctrh["bgtActMctr"]) || Convert.ToDecimal(ctrh["bgtActMctr"]) <= 0)
            {
                ctrh["bgtActMctr"] = ctrh["bgtestctr"];
            }
            if (BizFunctions.IsEmpty(ctrh["bgtActMsal"]) || Convert.ToDecimal(ctrh["bgtActMsal"]) <= 0)
            {
                ctrh["bgtActMsal"] = ctrh["bgtestsal"];
            }
            if (BizFunctions.IsEmpty(ctrh["bgtActMmat"]) || Convert.ToDecimal(ctrh["bgtActMmat"]) <= 0)
            {
                ctrh["bgtActMmat"] = ctrh["bgtestmat"];
            }
            if (BizFunctions.IsEmpty(ctrh["bgtActMchem"]) || Convert.ToDecimal(ctrh["bgtActMchem"]) <= 0)
            {
                ctrh["bgtActMchem"] = ctrh["bgtestchem"];
            }
            if (BizFunctions.IsEmpty(ctrh["bgtActMmach"]) || Convert.ToDecimal(ctrh["bgtActMmach"]) <= 0)
            {
                ctrh["bgtActMmach"] = ctrh["bgtestmach"];
            }
            if (BizFunctions.IsEmpty(ctrh["bgtActMequip"]) || Convert.ToDecimal(ctrh["bgtActMequip"]) <= 0)
            {
                ctrh["bgtActMequip"] = ctrh["bgtestequip"];
            }
            if (BizFunctions.IsEmpty(ctrh["bgtActMToi"]) || Convert.ToDecimal(ctrh["bgtActMToi"]) <= 0)
            {
                ctrh["bgtActMToi"] = ctrh["bgtestToi"];
            }
            if (BizFunctions.IsEmpty(ctrh["bgtActMPrd"]) || Convert.ToDecimal(ctrh["bgtActMPrd"]) <= 0)
            {
                ctrh["bgtActMPrd"] = ctrh["bgtestPrd"];
            }
            if (BizFunctions.IsEmpty(ctrh["bgtActMLand"]) || Convert.ToDecimal(ctrh["bgtActMLand"]) <= 0)
            {
                ctrh["bgtActMLand"] = ctrh["bgtestLand"];
            }
            if (BizFunctions.IsEmpty(ctrh["bgtActMSub"]) || Convert.ToDecimal(ctrh["bgtActMSub"]) <= 0)
            {
                ctrh["bgtActMSub"] = ctrh["bgtestSub"];
            }
            if (BizFunctions.IsEmpty(ctrh["bgtActMOther"]) || Convert.ToDecimal(ctrh["bgtActMOther"]) <= 0)
            {
            ...
            //if (BizFunctions.IsEmpty(ctrh["bgtsactctr"]) || Convert.ToDecimal(ctrh["bgtsactctr"]) <= 0)
            //{
            ...
            //if (BizFunctions.IsEmpty(ctrh["bgtsactLand"]) || Convert.ToDecimal(ctrh["bgtsactLand"]) <= 0)
            //{
            ...
            //}
            //if (BizFunctions.IsEmpty(ctrh["bgtsactSub"]) || Convert.ToDecimal(ctrh["bgtsactSub"]) <= 0)
            //{
            ...
            //}
            //if (BizFunctions.IsEmpty(ctrh["bgtsactOther"]) || Convert.ToDecimal(ctrh["bgtsactOther"]) <= 0)
            //{
            ...
            {
                ctrh["bgtlactsal"] = ctrh["bgtactsal"];
            }
            if (!BizFunctions.IsEmpty(ctrh["bgtActMPrd"]))
            {
        ...
        private void getBgtEstTotal()
        {
            ...
            decimal grandTotal = Convert.ToDecimal(ctrh["bgtestsal"]) + Convert.ToDecimal(ctrh["bgtestmat"]) + Convert.ToDecimal(ctrh["bgtestchem"]) +
                                 ...
                                 Convert.ToDecimal(ctrh["bgtestLand"]) + Convert.ToDecimal(ctrh["bgtestSub"]) + Convert.ToDecimal(ctrh["bgtestOther"]);
