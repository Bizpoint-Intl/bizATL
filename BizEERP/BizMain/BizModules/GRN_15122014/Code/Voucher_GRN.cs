        ...
        protected override void Document_SaveBegin_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            if (grnh["status"].ToString().Trim() == Common.DEFAULT_DOCUMENT_STATUSP)
            {
                if (grnh["type"] == "SITE")
                {
                    ...
                    foreach (DataRow dr in mwt.Rows)
                    {
                        if (dr.RowState != DataRowState.Deleted)
                        {
                            dr["guid"] = Tools.getGUID();
                ...
                else
                {
                    ...
                    DataTable GroupGldAcc = BizFunctions.ExecuteQuery(dbaccess.DataSet, "SELECT matnum,detail,qty*uomqty qty,uom,'' barcode FROM GRN1 where refnum ='" + grnh["refnum"] + "' and ISNULL(sn,0)=0"
                ...
                case "grnh_ponum":
                    ...
                    if (!ponum.Equals(""))
                    {
                        ...
                        if (porhTmp.Rows.Count != 0)
                        {
                            ...
                            string selectPor1 = "SELECT ponum,matnum,uom,discamt,detail,sum(qty) as qty,price,sn,uomcode,uomqty from " +
                                "(SELECT refnum as ponum, matnum,uom,discamt,detail, pqty qty,price,sn,uomcode,uomqty from por1 UNION ALL" +
                                " SELECT ponum, matnum,uom,discamt,detail,-qty as qty,0 as price,sn,uomcode,uomqty from grn1 WHERE refnum <>'" + grnh["refnum"].ToString().Trim() + "')a" +
                                "  WHERE ponum = '" + grnh["ponum"].ToString().Trim() + "' GROUP BY ponum,matnum,uom,discamt,detail,price,sn,uomcode,uomqty" +
                                " HAVING SUM(qty) >0";
