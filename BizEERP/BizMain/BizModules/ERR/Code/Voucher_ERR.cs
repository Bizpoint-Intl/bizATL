...
namespace ATL.ERR
{
    public class Voucher_ERR : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        protected override void Document_Form_OnLoad(object sender, DocumentEventArgs e)
        {
            ...
            e.DBAccess.DataSet.Tables["errh"].ColumnChanged += new DataColumnChangeEventHandler(Voucher_ERR_ColumnChanged);
            ...
            if (!BizFunctions.IsEmpty(errh["empnum"]))
            {
                ...
                if (BizFunctions.IsEmpty(errh["currentsal"]))
                {
                    errh["currentsal"] = GetCurrentSal();
                }
                else
                {
                    if (Convert.ToDecimal(errh["currentsal"]) == 0)
                    {
                        errh["currentsal"] = GetCurrentSal();
                    }
                }
        ...
        protected override void Document_SaveEnd_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            if (errh["status"].ToString() == (string)Common.DEFAULT_DOCUMENT_STATUSP)
            {
                ...
                ////To delete at current month + 6 days
                //// d = next Processing Day
                //    // d-1 = last processing day
                ////if current effective resignation Day < d-1 => Straignh away delay

                DateTime dt1 = Convert.ToDateTime(DateTime.Now.ToShortDateString());

                DateTime dt2 = Convert.ToDateTime(Convert.ToDateTime(errh["effectivedate"]).ToShortDateString());

                if (dt1 >= dt2)
                {
                    this.dbaccess.RemoteStandardSQL.ExecuteNonQuery("UPDATE HEMPH set statuscode = 'RESIGNED' where empnum='" + errh["empnum"].ToString() + "' ");
                    this.dbaccess.RemoteStandardSQL.ExecuteNonQuery("Delete from SITMT8 where empnum='" + errh["empnum"].ToString() + "' ");
                }
            ...
            if (errh["status"].ToString() == (string)Common.DEFAULT_DOCUMENT_STATUSV)
            {
                ...
                MessageBox.Show("Emp No " + errh["empnum"].ToString() + "'s Status has been set to 'ACTIVE'", "ATL Maintenance Pte. Ltd.", MessageBoxButtons.OK);
            }
        ...
        {
            DataTable SigTB = BizFunctions.ExecuteQuery(this.dbaccess.DataSet, "select empnum,userSignature as signaturepicloc,appraiserSignature as girosigLoc from errh");
        ...
        private void Voucher_ERR_ColumnChanged(object sender, DataColumnChangeEventArgs e)
        {
            string supv = "";
