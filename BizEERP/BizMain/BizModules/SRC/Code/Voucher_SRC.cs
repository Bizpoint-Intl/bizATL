	...
	public class Voucher_SRC : BizRAD.BizApplication.VoucherBaseHelper
	{
		...
		protected override void Document_Confirm_Handle(object sender, DocumentHandleEventArgs e)
		{
			...
			else
			{
				...
				#region steph - giv / siv posting for accnum3 - discamt
						...
						if (bankchgStrColumn.Length > 0)
						{
							...
							if (Convert.ToString(sumSivRow["acctype"]).Trim() == "2" || Convert.ToString(sumSivRow["acctype"]).Trim() == "3" || Convert.ToString(sumSivRow["acctype"]).Trim() == "4")
							{
								...
								for (int i = 0; i < bankchgStrColumn.Length; i++)
								{
									...
									{
										case "refnum":
											tmpgiv["refnum"] = sumSivRow["refnum"];
											break;
										case "arnum":
											tmpgiv["arnum"] = sumSivRow["arnum"];
											break;
										case "locno":
											tmpgiv["locno"] = recp["locno"];
											break;
										case "deptno":
											tmpgiv["deptno"] = recp["deptno"];
											break;
										case "period":
											tmpgiv["period"] = sumSivRow["period"];
											break;
										case "accnum":
											tmpgiv["accnum"] = sumSivRow["accnum"];
											break;
										case "trandate":
											tmpgiv["trandate"] = sumSivRow["trandate"];
											break;
										case "lgr":
											tmpgiv["lgr"] = "PIV";
											break;
										...
										case "exrate":
											...
											tmpgiv["exrate"] = Convert.ToDecimal(sumSivRow["exrate"]);
											break;
										case "oricur":
											tmpgiv["oricur"] = sumSivRow["oricur"];
											break;
										case "oriamt":
											...
											#endregion
											tmpgiv["oriamt"] = BizFunctions.Round((Convert.ToDecimal(sumSivRow["oriamt"])) * 100 / (100 + Convert.ToDecimal(sumSivRow["gstper"])));
											break;
										case "postamt":
											#region To replace the null value
											if (sumSivRow["exrate"] == System.DBNull.Value)
											...
											#endregion
											tmpgiv["postamt"] = BizFunctions.Round((Convert.ToDecimal(sumSivRow["oriamt"])) * 100 / (100 + Convert.ToDecimal(sumSivRow["gstper"])) * (Convert.ToDecimal(sumSivRow["exrate"])));
											break;
										case "gstper":
											tmpgiv["gstper"] = Convert.ToDecimal(sumSivRow["gstper"]);
											break;
										case "gstamt":
											tmpgiv["gstamt"] = BizFunctions.Round(Convert.ToDecimal(sumSivRow["oriamt"]) - Convert.ToDecimal(tmpgiv["oriamt"]));
											break;
										case "oridebit":
											if ((decimal)sumSivRow["oriamt"] > 0)
											{
												tmpgiv["oridebit"] = System.Math.Abs(Convert.ToDecimal(sumSivRow["oriamt"]));
											}
											else
											{
												tmpgiv["oridebit"] = 0;
										...
										case "oricredit":
											...
											{
												tmpgiv["oricredit"] = System.Math.Abs(Convert.ToDecimal(sumSivRow["oriamt"]));
											}
											break;
										default:
											tmpgiv[bankchgStrColumn[i]] = sumSivRow[bankchgStrColumn[i]];
											break;
									}
								}

								piv1.Rows.Add(tmpgiv.ItemArray);
							}
							#endregion
								...
								for (int i = 0; i < bankchgStrColumn.Length; i++)
								{
									switch (bankchgStrColumn[i])
									{
										case "refnum":
											...
											//steph 16 Apr 2010 - as giv and siv does not contain exramt column, 
				...
				#region steph - giv / siv posting for accnum4 - flexamt
				if (dbaccess.DataSet.Tables["recp"].Select().Length > 0)
				{
						...
						if (bankchgStrColumn.Length > 0)
						{
							...
							if (Convert.ToString(sumFlexRow["acctype"]).Trim() == "2" || Convert.ToString(sumFlexRow["acctype"]).Trim() == "3" || Convert.ToString(sumFlexRow["acctype"]).Trim() == "4")
							{
								...
								for (int i = 0; i < bankchgStrColumn.Length; i++)
								{
									...
									{
										case "refnum":
											tmpgiv["refnum"] = sumFlexRow["refnum"];
											break;
										case "arnum":
											tmpgiv["arnum"] = sumFlexRow["arnum"];
											break;
										case "locno":
											tmpgiv["locno"] = recp["locno"];
											break;
										case "deptno":
											...
											break;
										case "period":
											tmpgiv["period"] = sumFlexRow["period"];
											break;
										case "accnum":
											tmpgiv["accnum"] = sumFlexRow["accnum"];
											break;
										case "trandate":
											tmpgiv["trandate"] = sumFlexRow["trandate"];
											break;
										case "lgr":
											tmpgiv["lgr"] = "PIV";
											break;
										...
										case "exrate":
											...
											tmpgiv["exrate"] = Convert.ToDecimal(sumFlexRow["exrate"]);
											break;
										case "oricur":
											tmpgiv["oricur"] = sumFlexRow["oricur"];
											break;
										case "oriamt":
											tmpgiv["oriamt"] = BizFunctions.Round((Convert.ToDecimal(sumFlexRow["oriamt"])) * 100 / (100 + Convert.ToDecimal(sumFlexRow["gstper"])));
											break;
										case "postamt":
											tmpgiv["postamt"] = BizFunctions.Round((Convert.ToDecimal(sumFlexRow["oriamt"])) * 100 / (100 + Convert.ToDecimal(sumFlexRow["gstper"])) * (Convert.ToDecimal(sumFlexRow["exrate"])));
											break;
										case "gstper":
											tmpgiv["gstper"] = Convert.ToDecimal(sumFlexRow["gstper"]);
											break;
										case "gstamt":
											tmpgiv["gstamt"] = BizFunctions.Round(Convert.ToDecimal(sumFlexRow["oriamt"]) - Convert.ToDecimal(tmpgiv["oriamt"]));
											break;
										case "oridebit":
											...
											{
												tmpgiv["oricredit"] = System.Math.Abs(Convert.ToDecimal(sumFlexRow["oriamt"]));
											}
											break;
										case "oricredit":
											if ((decimal)sumFlexRow["oriamt"] < 0)
											...
											break;
										default:
											tmpgiv[bankchgStrColumn[i]] = sumFlexRow[bankchgStrColumn[i]];
											break;
									}
								}
                ...
                #region wenyue add for Eunice bank transfer to bank, because she dose not want to user Jouronal , Apr 2016
                for (int j = 0; j < siv1.Rows.Count; j++)
                {
                    string sqlCheckAcc = "select * from acm where accnum ='" + siv1.Rows[j]["accnum"] + "' and acctype=6";
                    DataSet dsCheckAcc = dbaccess.ReadSQLTemp("dtCheckAcc", sqlCheckAcc);
                    if (dsCheckAcc.Tables[0].Rows.Count > 0)
                    {
                        DataRow dr_Csh = this.dbaccess.DataSet.Tables["csh"].NewRow();
                        if (dr_Csh.RowState != DataRowState.Deleted)
                        {
                            dr_Csh["refnum"] = recp["refnum"];
                            dr_Csh["arnum"] = recp["arnum"];
                            dr_Csh["accnum"] = siv1.Rows[j]["accnum"];
                            dr_Csh["oricur"] = siv1.Rows[j]["oricur"];
                            dr_Csh["exrate"] = siv1.Rows[j]["exrate"];
                            dr_Csh["trandate"] = siv1.Rows[j]["trandate"];
                            dr_Csh["detail"] = recp["remark"];
                            dr_Csh["docunum"] = siv1.Rows[j]["docunum"];
                            dr_Csh["remark"] = siv1.Rows[j]["remark"];
                            dr_Csh["lgr"] = "CSH";
                            dr_Csh["flag"] = "MSRC";
                            dr_Csh["status"] = "O";
                            dr_Csh["user"] = recp["user"];
                            dr_Csh["oricredit"] = (decimal)siv1.Rows[j]["oriamt"] * -1;
                            dr_Csh["oridebit"] = 0;
                            dr_Csh["oriamt"] = siv1.Rows[j]["oriamt"];
                            dr_Csh["postamt"] = siv1.Rows[j]["oriamt"];
                            dr_Csh["bankamt"] = siv1.Rows[j]["oriamt"];
                            dr_Csh["exramt"] = 0;
                            dr_Csh["period"] = BizAccounts.GetPeriod(dbaccess, (DateTime)siv1.Rows[j]["trandate"]);
                        }
                        this.dbaccess.DataSet.Tables["csh"].Rows.Add(dr_Csh);
                    }
                }
		...
		protected override void Document_Save_Handle(object sender, DocumentHandleEventArgs e)
		{
            ...
            if (BizFunctions.IsEmpty(recp["remark"]))
            {
                recp["remark"] = recp["chknum"];
            }
		...
		protected override void Document_Form_OnLoad(object sender, DocumentEventArgs e)
		{
            ...
            if (recp["flag"].ToString().ToUpper().Trim() == "MSRC")
            {
                if (BizFunctions.IsEmpty(recp["arnum"]))
                {
                    recp["arnum"] = "MISC";
                }
            }
			...
			if (dbaccess.DataSet.Tables["readPosInfoFromArm"].Rows.Count > 0)
			{
				...
				if (readPosInfoFromArm["outletid"].ToString().Trim() != String.Empty)
				{
					...
					if ((bool)readPosInfoFromArm["isCoach"] == true)
					{
						...
						//strGetPOS = "SELECT sitenum,paytype,SUM(tenderamt) AS tenderamt,reason,OrderSNo  " +
						//    " FROM ( " +
						//    " SELECT trandate,sitenum,paytype,reason,OrderSNo,tenderamt FROM pos2 " +
						//    " UNION ALL " +
						//    " SELECT trandate,sitenum,detail2 as paytype,reason,OrderSNo,-aftdeposit AS tenderamt FROM saj1" +
						//    " UNION ALL " +
						//    " SELECT trandate,sitenum,detail2 as paytype,reason,OrderSNo,oriamt AS tenderamt FROM dard " +
						//    " WHERE invnum = 'POS' AND ISNULL(chkReverse,'')<>'Y'" +
						//    " ) posSaj " +
						//    " WHERE reason like '%COACH%' AND sitenum = '" + recp["outletid"].ToString().Trim() + "' " +
						...
						//    " GROUP BY sitenum,paytype,reason,OrderSNo HAVING SUM(tenderamt)<>0";
						#endregion

						#region Steph - for AR as coach
						strGetPOS = "SELECT posdate,sitenum,paytype,SUM(tenderamt) AS tenderamt,reason,OrderSNo  " +
							" FROM ( " +
							" SELECT CONVERT(datetime,CONVERT(nvarchar(8),pos2.trandate,112)) AS posdate,pos2.sitenum,pos2.paytype,pos2.reason, "+
							...
							" UNION ALL " +
							" SELECT CONVERT(datetime,CONVERT(nvarchar(8),posdate,112)) AS posdate,sitenum,detail2 as paytype,reason,OrderSNo,-aftdeposit AS tenderamt FROM saj1" +
							" UNION ALL " +
							" SELECT CONVERT(datetime,CONVERT(nvarchar(8),posdate,112)) AS posdate,sitenum,detail2 as paytype,reason,OrderSNo,oriamt AS tenderamt FROM dard " +
							" WHERE invnum = 'POS' AND ISNULL(chkReverse,'')<>'Y'" +
							" ) posSaj " +
							" WHERE reason like '%COACH%' " +
							//" AND sitenum = '" + recp["outletid"].ToString().Trim() + "' " +
					...
					else
					{
						...
						//    " WHERE invnum = 'POS' AND ISNULL(chkReverse,'')<>'Y'" +
						//    " ) posSaj " +
						//    " WHERE reason not like '%COACH%' AND sitenum = '" + recp["outletid"].ToString().Trim() + "' " +
						//    " AND convert(nvarchar(12),trandate,112) = '" + Convert.ToDateTime(recp["trandate"]).ToString("yyyyMMdd") + "' " +
						//    " AND ISNULL(paytype,'') in (SELECT ISNULL(paymentmode,'') FROM armpos WHERE arnum='" + recp["arnum"].ToString().Trim() + "')" +
						//    " GROUP BY sitenum,paytype,reason,OrderSNo HAVING SUM(tenderamt)<>0";
						#endregion
				...
				{
					dbaccess.ReadSQL("checkOutlet", "SELECT sitenum FROM sitm WHERE isnull(sitenum,'') = '" + recp["outletid"].ToString().Trim() + "'");
			...
			foreach (DataRow dr in getPos.Rows)
			{
				if (dr.RowState != DataRowState.Deleted)
				{
					...
					if (dr["reason"].ToString().Trim().Contains("Deposit"))
					{
						...
						newDard["sitenum"] = dr["sitenum"].ToString().Trim();  //steph - added this to check the paymode extracted before
					...
					else
					{
						...
						newSaj1["sitenum"] = dr["sitenum"].ToString().Trim();  //steph - added this to check the paymode extracted before
						...
						//string strGetPosRefnum = "SELECT POSResult.refnum,SUM(POSResult.tenderamt - POSResult.change) AS tenderamt,POSResult.sitenum,POSResult.reason " +
						//    " FROM " +
						//    " (SELECT * FROM POS2 " +
						//    " WHERE CONVERT(datetime,Convert(nvarchar(8),trandate,112)) = '" + Convert.ToDateTime(dr["posdate"]).ToString("yyyyMMdd").Trim() + "' " +
						//    " AND sitenum = '" + dr["sitenum"].ToString().Trim() + "' AND paytype = '" + dr["paytype"].ToString().Trim() + "' " +
						//    " ) POSResult " +
						//    " LEFT OUTER JOIN posh ON posh.refnum = POSResult.refnum " +
						//    " GROUP BY POSResult.refnum,POSResult.sitenum,POSResult.reason";


						string strGetPosRefnum = "SELECT POSResult.refnum,SUM(POSResult.tenderamt - POSResult.change) AS tenderamt, "+
							" POSResult.sitenum,POSResult.reason " +
							" FROM " +
							" (SELECT pos2.refnum,pos2.tenderamt,pos2.change,pos2.sitenum,pos2.reason,"+
							...
							" LEFT OUTER JOIN (SELECT * FROM arm WHERE ISNULL(iscoach,0)=0) arm ON ISNULL(arm.outletid,'') = ISNULL(pos2.sitenum,'') "+
							" WHERE CONVERT(datetime,Convert(nvarchar(8),POS2.trandate,112)) = '" + Convert.ToDateTime(dr["posdate"]).ToString("yyyyMMdd").Trim() + "' " +
							" AND POS2.sitenum = '" + dr["sitenum"].ToString().Trim() + "' "+
		...
		protected override void Document_Reopen_Handle(object sender, DocumentHandleEventArgs e)
		{
            ...
            if (BizValidate.ChkPeriodLocked(e.DBAccess, RECP["period"].ToString()))
            {
			...
			switch(e.Column.ColumnName)
			{
				...
				case "gstgrpnum":
					...
					if(this.dbaccess.DataSet.Tables["gstm"].Rows.Count != 0)
					{
                        ...
                        {
                            if (BizFunctions.IsEmpty(recp["trandate"]))
                                recp ["trandate"] = DateTime.Today;
		...
		private void AutoCalc()
		{
            ...
            decimal checkttl = 0;
            ArTotal = ard_postamt;
            Siv1Total = siv1_postamt + siv1_gstamt;

            if (recp["status"].ToString() != Common.DEFAULT_DOCUMENT_STATUSP) // if confim only post into ard ,csh and gld
            {
                CshPaid = BizFunctions.Round((Convert.ToDecimal(recp["amtrcv"]) + Convert.ToDecimal(recp["flexamt"])
                    + Convert.ToDecimal(recp["flexamt"])) * Convert.ToDecimal(recp["exrate"]));

                steDiff = ArTotal + Siv1Total - CshPaid;
                checkttl = (decimal)recp["oriamt"] - (decimal)recp["amtrcv"];
                txtRecStatus.Text = "AR Rounding = " + steDiff.ToString().Trim();
                if (checkttl != 0)
                {
                    check = false;   //globe permiter
                }
                else
                {
                    check = true;
