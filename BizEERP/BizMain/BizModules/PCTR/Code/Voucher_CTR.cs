...
namespace ATL.PCTR
{
    public class Voucher_PCTR : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        public Voucher_PCTR(string moduleName, Hashtable voucherBaseHelpers)
        ...
        protected override void Document_Form_OnLoad(object sender, DocumentEventArgs e)
        {
            ...
            DataTable CTR18 = e.DBAccess.DataSet.Tables["CTR18"];
            ...
            if (!BizFunctions.IsEmpty(CTRH["status"]))
            {
                if (CTRH["status"].ToString().Trim() == Common.DEFAULT_DOCUMENT_STATUSN)
                {
                    if (CTR18.Rows.Count == 0)
                    {
                        
                            DataRow insertCtr18_1 = CTR18.NewRow();
                            insertCtr18_1["svccode"] = "P1";
                            insertCtr18_1["frequencycode"] = "MONTHLY";

                            CTR18.Rows.Add(insertCtr18_1);

                            DataRow insertCtr18_2 = CTR18.NewRow();
                            insertCtr18_2["svccode"] = "P1";
                            insertCtr18_2["frequencycode"] = "MONTHLY";

                            CTR18.Rows.Add(insertCtr18_2);
                    }
                }
            }
            ...
            e.DBAccess.DataSet.Tables["CTR18"].ColumnChanged += new DataColumnChangeEventHandler(Voucher_CTR18_ColumnChanged);
        ...
        Document_Save_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            foreach (DataRow dr18 in ctr18.Rows)
            {
                if (dr18.RowState != DataRowState.Deleted)
                {
                    ...
                    {
                        if (BizFunctions.IsEmpty(dr18["sitenum"]))
                        {
                            dr18["sitenum"] = ctrh["sitenum"];
                        }
                    }
                }
            }
        ...
        protected override void AddDocumentF2Condition(object sender, DocumentF2EventArgs e)
        {
            ...
            switch (e.ControlName)
            {
                case "ctrh_arnum":
                    ...
                    {
                        e.DefaultCondition = "(arm.arnum like '%" + ctrh["arnum"].ToString().Trim() + "%' OR arm.arname like '" + ctrh["arnum"].ToString().Trim() + "%') and arm.[status]<>'V' and isnull(iscoach,0)=1";
                   //     e.Condition = BizFunctions.F2Condition("arnum,arname", (sender as TextBox).Text);
                ...
                case "ctrh_sitenum":
                    ...
                    //    e.Condition = BizFunctions.F2Condition("sitenum,sitename", (sender as TextBox).Text);
                        ...
                        if (ctrh["sitenum"].ToString() == string.Empty)
                        {
                            e.DefaultCondition = " sitm.[status]<>'V' and isnull(iscoach,0)=1";
                        }
                        else
                        {
                            e.DefaultCondition = "(sitm.sitenum like '" + ctrh["sitenum"].ToString() + "%' ) and sitm.[status]<>'V' and isnull(iscoach,0)=1";
                        }
                    }
                    else
                    {
        ...
        protected override void AddDetailF2Condition(object sender, DetailF2EventArgs e)
        {
            ...
            switch (e.MappingName)
            {
                ...
                case "sitenum":
                    e.Condition = BizFunctions.F2Condition("sitenum,sitename", (sender as TextBox).Text);
                    e.DefaultCondition = " arnum in "+
                                        "( "+
                                        "select arnum from arm where parnum='"+CTRH["arnum"].ToString()+"' "+
                                        ")";
                    break;

                case "empnum":
                    e.Condition = BizFunctions.F2Condition("empnum,empname", (sender as TextBox).Text);
                    e.DefaultCondition = " jobgroup='PC' ";
                    break;
        ...
        protected override void AddDetailF3Condition(object sender, DetailF3EventArgs e)
        {
            ...
            switch (e.MappingName)
            {
                ...
                case "empnum":
                    {
                        e.CurrentRow["empname1"] = e.F2CurrentRow["empname"];
                    }
                    break;
                case "empnum2":
                    {
                        e.CurrentRow["empname2"] = e.F2CurrentRow["empname"];
                    }
                    break;
                case "empnum3":
                    {
                        e.CurrentRow["empname3"] = e.F2CurrentRow["empname"];
                    }
                    break;
        ...
        protected override void Document_SaveBegin_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            foreach (DataRow dr in ctr1.Rows)
            {
                if (dr.RowState != DataRowState.Deleted)
                {

                    BizFunctions.UpdateDataRow(ctrh, dr, "trandate/user/flag/status/created/modified/SystemYear");
                }
            }
            ...
            foreach (DataRow dr18 in ctr18.Rows)
            {
                if (dr18.RowState != DataRowState.Deleted)
                {
                    BizFunctions.UpdateDataRow(ctrh, dr18, "refnum/user/flag/status/created/modified");
                }
            }
        ...
        private void Voucher_CTR18_ColumnChanged(object sender, DataColumnChangeEventArgs e)
        {
            switch (e.Column.ColumnName)
            {
                case "svccode":
                    {
                        //if (isDuplicate)
                        //{
                        //    e.Row["guid"] = System.DBNull.Value;
                        //}
