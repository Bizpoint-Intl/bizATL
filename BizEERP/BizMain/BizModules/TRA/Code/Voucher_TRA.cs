...
namespace ATL.TRA
{
    public class Voucher_TRA : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        {
            DataRow payt = dbaccess.DataSet.Tables["trah"].Rows[0];
            #region Steph - Setting of Default Value
            MDTReader.SetDefaultValueNVC(ref this.dbaccess, payt["flag"].ToString().Trim(), "trah", "accnum");
        ...
        private void setTraMonth()
        {
            ...
            if (!BizFunctions.IsEmpty(trah["senddate"]))
            {
                if (BizFunctions.IsEmpty(trah["tramonth"]))
                {
                    trah["tramonth"] = Convert.ToDateTime(trah["senddate"]).Month;
                }
                else
                {
                    if (Convert.ToInt16(trah["tramonth"]) == 0)
                    {
                        trah["tramonth"] = Convert.ToDateTime(trah["senddate"]).Month;
                    }
                }
                ...
                switch (e.Column.ColumnName)
                {
                    case "matnum":
                        ...
                        if (tmpds.Tables["matm"].Rows.Count != 0)
                        {
                            ...
                            e.Row["matname"] = tmpds.Tables["matm"].Rows[0]["matname"].ToString().Trim();
                            e.Row["accnum"] = tmpds.Tables["matm"].Rows[0]["purcacc"].ToString().Trim();
        ...
        protected override void Document_Save_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            if (BizFunctions.IsEmpty(trah["ccnum"]))
            {
                trah["ccnum"] = "CLN";
            }
            foreach (DataRow dr1 in tra1.Rows)
            ...
            if (tra1.Rows.Count > 0)
            {
                foreach (DataRow dr1 in tra1.Rows)
                {
                    if (dr1.RowState != DataRowState.Deleted)
                    {
                        ...
                        if (!BizFunctions.IsEmpty(dr1["tositenum"]) && BizFunctions.IsEmpty(dr1["towhnum"]))
                        {
                            string towhnum = BizLogicTools.Tools.GetWhnum(dr1["tositenum"].ToString(), this.dbaccess);
                            if (towhnum != string.Empty)
                            {
                                dr1["towhnum"] = towhnum;
                            }
                            else
                            {
                                dr1["towhnum"] = dr1["tositenum"];
                            }
                        }
        ...
        protected override void AddDocumentF2Condition(object sender, DocumentF2EventArgs e)
        {
            ...
            switch (e.ControlName)
            {
                ...
                case "trah_tositenum":
                    ...
                    break;
                case "trah_trqnum":
                    if (trah["flag"].ToString().Trim().ToUpper() == "TRA")
                    {
                        e.DefaultCondition = " refnum not in (Select ISNULL(trqnum,'') as refnum  from TRAH where [status]<>'V') and flag='TRQ'   ";
                    }
                    else
                    {
                        e.DefaultCondition = " refnum not in (Select ISNULL(trqnum,'') as refnum  from TRAH where [status]<>'V') and flag='LTRQ'   ";
                    }
                ...
                case "trah_trqnum":
                    {
                        ...
                        {
                            trah["ccnum"] = GetCcnumFromTRQ(trah["trqnum"].ToString().Trim());
                            trah["projectid"] = GetProjectIDFromTRQ(trah["projectid"].ToString().Trim());
                        }
                        ...
                        if (trah["flag"].ToString().Trim().ToUpper() == "LTRA")
                        {
                            trah["requiredDateFrom"] = e.F2CurrentRow["requiredDateFrom"];
                            trah["requiredDateTo"] = e.F2CurrentRow["requiredDateTo"];
                        }
                    }
                    break;
        ...
        protected override void Document_SaveBegin_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            foreach (DataRow dr in tra1.Rows)
            {
                if (dr.RowState != DataRowState.Deleted)
                {
                    ...
                    BizFunctions.UpdateDataRow(trah, dr, "trandate/user/flag/status/created/modified");
                }
            }
        ...
        protected override void Document_Refresh_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            foreach (DataRow dr1 in tra1.Rows)
            {
                if (dr1.RowState != DataRowState.Deleted)
                {
                    //Amended by Spencer - Clarence can change Issue Qty
                    if (dr1["qty"] == System.DBNull.Value)
                    {
                        dr1["qty"] = dr1["traqty"];
                    }
                    ...
                    if (!BizFunctions.IsEmpty(dr1["matnum"]))
                    {
                        ...
                        {
                            //dr1["retail"] = BizLogicTools.Tools.GetPeriodCost(dr1["matnum"].ToString(), BizAccounts.GetPeriod(this.dbaccess, Convert.ToDateTime(trah["trandate"])), this.dbaccess);
                    ...
                    if (trah["searchType"].ToString().Trim() == "SITM")
                    {
                        if (!BizFunctions.IsEmpty(trah["tositenum"]))
                        {
        ...
        protected override void Document_Confirm_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            else
            {
                ...
                DataTable getEmptyAcPivh = BizFunctions.ExecuteQuery(dbaccess.DataSet, "SELECT accnum from [trah] WHERE isnull(accnum,'') not in (SELECT accnum from [checkAcm])");
                if (getEmptyAcPivh.Rows.Count > 0)
                {
                    DataEntryErrors = DataEntryErrors + "\n Invalid A/C No. (Credit A/C) in Header";
                }

                DataTable getEmptyAcPiv1 = BizFunctions.ExecuteQuery(dbaccess.DataSet, "SELECT refnum from [tra1]  WHERE isnull(accnum,'') not in (SELECT accnum FROM [checkAcm])");
                ...
                {
 //                   DataTable GroupPivcAcc = BizFunctions.ExecuteQuery(dbaccess.DataSet, "SELECT accnum,sum(isnull(wac,0)) as oriamt, sum(isnull(wac,0)) as postamt,detail  from [tra1] left join wac"+Common.DEFAULT_SYSTEM_YEAR+" wac on tra1.matnum=wac.matnum"
 //+" where wac.period=1 group by accnum,detail");


                    string GroupPivcAccStr = "SELECT accnum,sum(isnull(wac,0)) as oriamt, sum(isnull(wac,0)) as postamt,detail  from [tra1] left join wac"+Common.DEFAULT_SYSTEM_YEAR+" wac on tra1.matnum=wac.matnum"
                                             +" where wac.period=1 and [tra1].refnum='"+trah["refnum"].ToString().Trim()+"' group by accnum,detail";

                    this.dbaccess.ReadSQL("GroupPivcAcc", GroupPivcAccStr);

                    DataTable GroupPivcAcc = this.dbaccess.DataSet.Tables["GroupPivcAcc"];


                    // wenyue comment until WAC have value

                    //foreach (DataRow dr in GroupPivcAcc.Rows)
                    //{
                    //    DataRow addGL = gld.Rows.Add(new object[] { });
                    //    addGL["accnum"] = dr["accnum"];
                    //    addGL["ccnum"] = dr["ccnum"];
                    //    addGL["detail"] = dr["detail"];
                    //    addGL["oricur"] = "SGD";
                    //    addGL["exrate"] = 1;
                    //    if (Convert.ToDecimal(trah["oriamt"]) > 0)
                    //    {
                    //        addGL["oricredit"] = 0;
                    //        addGL["oridebit"] = Math.Abs(Convert.ToDecimal(trah["oriamt"]));
                    //    }
                    //    else
                    //    {
                    //        addGL["oricredit"] = Math.Abs(Convert.ToDecimal(trah["oriamt"]));
                    //        addGL["oridebit"] = 0;
                    //    }
                    //    addGL["oriamt"] = trah["oriamt"];
                    //    addGL["postamt"] = trah["postamt"];
                    //    addGL["lgr"] = "TRA";



                    //    DataRow addGL1 = gld.Rows.Add(new object[] { });
                    //    addGL1["accnum"] = trah["accnum"];
                    //    addGL1["ccnum"] = dr["ccnum"];
                    //    addGL1["detail"] = dr["detail"];
                    //    addGL1["oricur"] = "SGD";
                    //    addGL1["exrate"] = 1;
                    //    if (Convert.ToDecimal(trah["oriamt"]) > 0)
                    //    {
                    //        addGL1["oricredit"] = Math.Abs(Convert.ToDecimal(trah["oriamt"]));
                    //        addGL1["oridebit"] = 0;
                    //    }
                    //    else
                    //    {
                    //        addGL1["oricredit"] = 0;
                    //        addGL1["oridebit"] = Math.Abs(Convert.ToDecimal(trah["oriamt"]));
                    //    }
                    //    addGL1["oriamt"] = trah["oriamt"];
                    //    addGL1["postamt"] = trah["postamt"];
                    //    addGL1["lgr"] = "STK";
                    //}
                }
            }
        ...
        private void GetByTRQ(string trqnum)
        {
            ...
            string getTRQ = "select " +
                                ...
                                "SUM(stdqty) as stdqty ,senddate,uomcode,ponum,ponum2,ponum3,ponum4,whnum" +
                            " from TRQ1 " +
                            "Where refnum='" + trqnum + "' " +
                            "GROUP BY refnum,sectorcode,sitenum,matnum,barcode,senddate,uomcode,ponum,ponum2,ponum3,ponum4,whnum";
            ...
            if (MatTB != null)
            {
                if (MatTB.Rows.Count > 0)
                {
                    foreach (DataRow dr1 in MatTB.Rows)
                    {
                        ...
                        if ((!BizFunctions.IsEmpty(dr1["ponum"]) || !BizFunctions.IsEmpty(dr1["ponum2"]) || !BizFunctions.IsEmpty(dr1["ponum3"])) && (Math.Abs(Convert.ToDecimal(dr1["trqqty"]) - BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum"].ToString(), this.dbaccess)) > 0))
                        {
                            //decimal test1 = BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum"].ToString(), this.dbaccess);
                            //decimal test2 = Math.Abs(Convert.ToDecimal(dr1["trqqty"]) - (BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum"].ToString(), this.dbaccess) + BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum2"].ToString(), this.dbaccess) + BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum3"].ToString(), this.dbaccess)));


                            //decimal value1 = BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum"].ToString(), this.dbaccess);
                            //decimal value2 = BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum2"].ToString(), this.dbaccess);
                            //decimal value3 = BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum3"].ToString(), this.dbaccess);
                            //decimal value4 = Convert.ToDecimal(dr1["qty"]);
                                          

                            DataRow InsertTRA1 = TRA1.NewRow();
                            InsertTRA1["docunum"] = dr1["refnum"];
                            InsertTRA1["ponum"] = dr1["ponum"];
                            InsertTRA1["ponum2"] = dr1["ponum2"];
                            InsertTRA1["ponum3"] = dr1["ponum3"];
                            InsertTRA1["ponum4"] = dr1["ponum4"];
                            InsertTRA1["sectorcode"] = dr1["sectorcode"];
                            InsertTRA1["sitenum"] = dr1["sitenum"];
                            InsertTRA1["matnum"] = dr1["matnum"];
                            InsertTRA1["uom"] = dr1["uomcode"];
                            InsertTRA1["trqqty"] = dr1["qty"];
                            InsertTRA1["traqty"] = Math.Abs(Convert.ToDecimal(dr1["trqqty"]) - (BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum"].ToString(), this.dbaccess) + BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum2"].ToString(), this.dbaccess) + BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum3"].ToString(), this.dbaccess) + BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum4"].ToString(), this.dbaccess)));                   
                            InsertTRA1["lastdate"] = dr1["senddate"];
                            InsertTRA1["stdqty"] = dr1["stdqty"];
                            InsertTRA1["retail"] = dr1["retail"];
                            InsertTRA1["qty"] = Math.Abs(Convert.ToDecimal(dr1["trqqty"]) - (BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum"].ToString(), this.dbaccess) + BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum2"].ToString(), this.dbaccess) + BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum3"].ToString(), this.dbaccess) + BizLogicTools.Tools.GetIssueDifference(dr1["matnum"].ToString(), dr1["ponum4"].ToString(), this.dbaccess)));                   
                            InsertTRA1["barcode"] = dr1["barcode"];
                            InsertTRA1["lasttotalqty"] = dr1["lasttotalqty"];
                            InsertTRA1["whnum"] = dr1["whnum"];
                            TRA1.Rows.Add(InsertTRA1);
                            
                        }
                        else if(BizFunctions.IsEmpty(dr1["ponum"]))
                        {
                            DataRow InsertTRA1 = TRA1.NewRow();
                            InsertTRA1["docunum"] = dr1["refnum"];
                            InsertTRA1["ponum"] = dr1["ponum"];
        ...
        void btn_ExtractTemplate_Click(object sender, EventArgs e)
        {
            ...
            if (!BizFunctions.IsEmpty(TRAH["searchType"]))
            {
                ...
                else if (TRAH["searchType"].ToString() == "TRQ")
                {
                        ...
                        ATL.TRA.ExtractTRQ form = new ATL.TRA.ExtractTRQ(this.dbaccess, tra1);
                        form.Show();
                        form.Focus();
        ...
        private string GetCcnumFromTRQ(string trqnum)
        {
            string ccnum = "";
            string get1 = "Select ccnum from trqh where refnum='"+trqnum+"' ";

            this.dbaccess.ReadSQL("TempCcnuTrq", get1);

            DataTable dt = this.dbaccess.DataSet.Tables["TempCcnuTrq"];
        ...
        private string GetProjectIDFromTRQ(string trqnum)
        {
            string projectid = "";
            string get1 = "Select projectid from trqh where refnum='" + trqnum + "' ";

            this.dbaccess.ReadSQL("TempProjectIDTrq", get1);

            DataTable dt = this.dbaccess.DataSet.Tables["TempProjectIDTrq"];
