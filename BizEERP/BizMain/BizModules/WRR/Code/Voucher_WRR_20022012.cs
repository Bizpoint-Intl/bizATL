...
namespace PicoGuards.WRR
{
    public class Voucher_WRR : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        protected override void Document_Save_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            //if (wrr1.Rows.Count > 0 || wrr2.Rows.Count > 0 || wrr3.Rows.Count > 0 || wrr4.Rows.Count > 0)
            //{
            //    if (scheduleChanged)
            //    {
            //        MessageBox.Show("You made Changes with the Schedule, Please Summarize before Saving, Unable to proceed", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            //        e.Handle = false;
            //    }
            //}
        ...
        private DataSet WR1()
        {
            ...
            selectedCollection.Add("SITMI", "Select * from SITMI where sitenumi='" + wrr["sitenumi"].ToString() + "'");
        ...
        private void updateATMR()
        {
            ...
            string FinalSchedule = "Select FW.REFNUM,FW.TB,FW.EMPNUM,FW.NRIC,FW.SHIFTCODE,FW.[DAY],FW.TIMEIN,FW.[TIMEOUT],FB.[DATE],FW.REMARK,FW.isRealSchedule FROM FinalAMTR FW " +
        ...
        {
            string sql1 = "";
            sql1 = "SELECT M1.refnum,M1.TB,M1.EMPNUM,M1.NRIC, M1.shiftcode, [day], vS.TIMEIN,vS.[TIMEOUT],M1.remark,M1.isRealSchedule " +
                "FROM " +
                "( " +
                    ...
                    if (dr1.RowState != DataRowState.Deleted)
                    {
                        foreach (DataRow dr2 in CurrentSchedule.Rows)
                        {
                            if (dr2.RowState != DataRowState.Deleted)
                            {
                                ...
                                if (dr1["empnum"].ToString() == dr2["empnum"].ToString() && dr1["shiftcode"].ToString() == dr2["shiftcode"].ToString() && BizFunctions.GetSafeDateString(Convert.ToDateTime(dr1["Date"])) == BizFunctions.GetSafeDateString(Convert.ToDateTime(dr2["Date"])) && dr1["isRealSchedule"] == dr2["isRealSchedule"])
                                {
