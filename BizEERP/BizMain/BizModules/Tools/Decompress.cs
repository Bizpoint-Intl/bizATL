...
{
    public partial class Decommpress : Form
    {
        DBAccess dbaccess = null;
        byte[] buffer;
        String temploc,TableName = "";
        public Decommpress(DBAccess dbaccess,string Tname)
        {
                ...
                if (TableNameTmp.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in TableNameTmp.Rows)
                    {
                        if (dr.RowState != DataRowState.Deleted)
                        {
                            ...
                            string sZipFileName = dr["nric"].ToString() + "-" + dr["empnum"].ToString() + "-" + BizFunctions.GetSafeDateString(dt);

                            // check to see if zipped file already exists
                            // user may rename it in the text box if it does.

                            //FileInfo fi = new FileInfo(txtSaveTo.Text + "\\" + sZipFileName + "-" + "001" + "\\");
                            System.IO.DirectoryInfo fl = new DirectoryInfo(txtSaveTo.Text + "\\" + sZipFileName + "-" + "001" + "\\");

                            string currentFilename = sZipFileName + "-" + "001";
                            while (fl.Exists)
                            {
