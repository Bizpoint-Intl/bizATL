    ...
    public class Voucher_ATR : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        private void Btn_Extract_Click(object sender, EventArgs e)
        {
            ...
            if (!BizFunctions.IsEmpty(atr["wrrnum"].ToString()))
            {
                ...
                try
                {
                    ...
                    ExtractALiveForm ExtractAL = new ExtractALiveForm(this.dbaccess, oriTable);
        ...
        {           
            ...
            switch (e.ControlName)
            {
                case "atr_wrrnum":
                    {
                       atr["Sitenumi"] = e.F2CurrentRow["sitenum"];
        ...
        protected override void Document_SaveBegin_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            foreach (DataRow dr1 in atr1.Rows)
            {
                if (dr1.RowState != DataRowState.Deleted)
                {
                    BizFunctions.UpdateDataRow(atr, dr1, "refnum/sitenum/sectorcode/wrrnum/user/flag/status/trandate/created/modified");
                ...
                foreach (DataRow dr1 in TBGetATMR.Select())
                {
                    if (dr1.RowState != DataRowState.Deleted)
                    {
                        ...
                        if (!BizFunctions.IsEmpty(dr1["timein"]) && !BizFunctions.IsEmpty(dr1["actualtimein2"]) || (dr1["timein"].ToString() != string.Empty && dr1["actualtimein2"] != string.Empty))
                        {
                                ...
                                //LocalTime actualtimeout = ASOMS.TimeUtilites.TimeTools.ParseMilitaryTime(TimeUtilites.TimeTools.GetSafeMilitaryTimeOnly1(Convert.ToDateTime(dr1["actualtimein"])));
                        ...
                        if (!BizFunctions.IsEmpty(dr1["timeout"]) && !BizFunctions.IsEmpty(dr1["actualtimeout2"]) || (dr1["timeout"].ToString() != string.Empty && dr1["actualtimeout2"].ToString() != string.Empty))
                        {
                                ...
                                //LocalTime actualtimeout = ASOMS.TimeUtilites.TimeTools.ParseMilitaryTime(TimeUtilites.TimeTools.GetSafeMilitaryTimeOnly1(Convert.ToDateTime(dr1["actualtimeout"])));
