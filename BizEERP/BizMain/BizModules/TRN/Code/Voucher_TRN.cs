...
namespace ATL.TRN
{
    public class Voucher_TRN : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        protected DateTimePicker trnh_trandate = null;
        ...
        protected override void AddVoucherDefaultCondition(VoucherConditionEventArgs e)
        {
            ...
            e.Condition = " [status]='O' and flag='"+flag+"' ";
        ...
        private void Initialise()
        {
            ...
            trnh_trandate = BizXmlReader.CurrentInstance.GetControl(headerFormName, "trnh_trandate") as DateTimePicker;

            trnh_trandate.Enabled = false;
        ...
        protected override void Document_Save_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            if (BizFunctions.IsEmpty(TRNH["BondDuration"]))
            {
                TRNH["bondStartDate"] = System.DBNull.Value;
                TRNH["bondEndDate"] = System.DBNull.Value;
            }
                ...
                case "trnh_trnqref":
                    {
                        ...
                        //e.DefaultCondition = " trnqref NOT IN (SELECT trnqref FROM TRNH WHERE [status]<>'V') ";
                ...
                case "trnh_pivnum":
                    {
                        ...
                        e.DefaultCondition = " pivhView.apnum like 'TV%' and (pivhView.[status]='O' OR pivhView.[status]='P') and pivhView.refnum not in (Select pivnum as refnum from trnh where [status]<>'V' and pivnum is not null)  ";
        ...
        protected override void AddDocumentF3Condition(object sender, DocumentF3EventArgs e)
        {
            ...
            switch (e.ControlName)
            {
                ...
                case "trnh_pivnum":
                    {
                        string getPivAmt = "Select postamt,gstamt from pivh where refnum='" + TRNH["pivnum"].ToString().Trim() + "' ";
                        this.dbaccess.ReadSQL("TempPIVH", getPivAmt);
                        DataTable TempPIVH = this.dbaccess.DataSet.Tables["TempPIVH"];

                        if (TempPIVH != null)
                        {
                            if (TempPIVH.Rows.Count > 0)
                            {
                                TRNH["NetPayment"] = Convert.ToDecimal(TempPIVH.Rows[0]["postamt"]);
                                TRNH["gstamt"] = Convert.ToDecimal(TempPIVH.Rows[0]["gstamt"]);
                            }
                        }
        ...
        private void GetTrainingRequestDetails()
        {
            DataRow trnh = this.dbaccess.DataSet.Tables["trnh"].Rows[0];
            DataTable trn1 = this.dbaccess.DataSet.Tables["trn1"];
            ...
            string getTrnh = "Select * from trnh where refnum='" + trnh["trnqref"].ToString().Trim() + "'";
            string getTrn1 = "Select * from trn1 where refnum='" + trnh["trnqref"].ToString().Trim() + "'";


            this.dbaccess.ReadSQL("TrnqH", getTrnh);
            this.dbaccess.ReadSQL("Trnq1", getTrn1);


            DataTable TrnqH = this.dbaccess.DataSet.Tables["TrnqH"];
            DataTable Trnq1 = this.dbaccess.DataSet.Tables["Trnq1"];

            if (TrnqH != null)
            {
                if (TrnqH.Rows.Count > 0)
                {
                    DataRow drTrnqH = this.dbaccess.DataSet.Tables["TrnqH"].Rows[0];

                    trnh["coursecode"] = drTrnqH["coursecode"];
                    trnh["coursename"] = drTrnqH["coursename"];
                    trnh["appraisedcomments"] = drTrnqH["appraisedcomments"];
                    trnh["appraisedby"] = drTrnqH["appraisedby"];
                    trnh["isrecommended"] = drTrnqH["isrecommended"];
     
                    trnh["trStartDate"] = drTrnqH["trPrefStartDate"];
                    trnh["trEndDate"] = drTrnqH["trPrefEndDate"];
                }
            }
            ...
            if (Trnq1 != null)
            {
                if (Trnq1.Rows.Count > 0)
                {
                    if (trn1.Rows.Count > 0)
                    {
                        BizFunctions.DeleteAllRows(trn1);
                    }

                    foreach (DataRow dr1 in Trnq1.Rows)
                    {
                        DataRow insertTrn1 = trn1.NewRow();
                        insertTrn1["nric"] = dr1["nric"];
                        insertTrn1["empnum"] = dr1["empnum"];
                        insertTrn1["empname"] = dr1["empname"];

                        trn1.Rows.Add(insertTrn1);
