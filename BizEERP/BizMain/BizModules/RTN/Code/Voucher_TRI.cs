...
namespace ATL.RTN
{
    public class Voucher_RTN : BizRAD.BizApplication.VoucherBaseHelper
    {
        #region Variable
        ...
        string sitenum, flag, towhnum,fromwhnum = null;
        ...
        public Voucher_RTN(string moduleName, Hashtable voucherBaseHelpers)
        ...
        protected override void AddVoucherAllCondition(VoucherConditionEventArgs e)
        {
            ...
            e.Condition = "trih.flag='" + moduleName + "' AND trih.systemyear = " + Common.DEFAULT_SYSTEM_YEAR;
        ...
        protected override void AddVoucherDefaultCondition(VoucherConditionEventArgs e)
        {
            ...
            e.Condition = " (trih.status= '" + Common.DEFAULT_DOCUMENT_STATUSN + "' or " +
                     ...
                     " AND trih.flag='" + moduleName + "' AND trih.systemyear = " + Common.DEFAULT_SYSTEM_YEAR;
        ...
        protected override void Document_Save_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            if (fromwhnum == string.Empty || fromwhnum == null)
            {
                if (!BizFunctions.IsEmpty(trih["fromsitenum"]))
                {
                    fromwhnum = getWhnum(trih["fromsitenum"].ToString());
                }
            }
            if (towhnum == string.Empty || towhnum == null)
            {
                if (!BizFunctions.IsEmpty(trih["tositenum"]))
                {
                    towhnum = getWhnum(trih["tositenum"].ToString());
                }
            }
        ...
        protected override void AddDocumentF2Condition(object sender, DocumentF2EventArgs e)
        {
            ...
            switch (e.ControlName)
            {
                ...
                case "trah_tranum":
                    e.DefaultCondition = " refnum not in (Select ISNULL(tranum,'') as refnum  from TRIH where [status]<>'V')   ";
                    break;
            }
        }
                    ...
                    if (trih["status"].ToString().Trim() == Common.DEFAULT_DOCUMENT_STATUSP)
                    {
                        ...
                        for (int i = 0; i <= 1; i++)
                        {
                            ...
                            if (i == 1)
                            {
                                ...
                                dr_mwt["qty"] = -(decimal)dr["acceptedqty"];
                                dr_mwt["whnum"] = fromwhnum;//Decrease qty in Stock In Transit warehouse(Location HQ)
                                dr_mwt["guid"] = Tools.getGUID();
                                dr_mwt["trandate"] = dr["trandate"];
                                dr_mwt["year"] = dr["year"];
                                dr_mwt["created"] = dr["created"];
