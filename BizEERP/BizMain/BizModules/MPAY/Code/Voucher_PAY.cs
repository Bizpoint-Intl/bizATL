...
namespace ATL.MPAY
{
	public class Voucher_MPAY : BizRAD.BizApplication.VoucherBaseHelper
	{
		...
		public Voucher_MPAY(string moduleName, Hashtable voucherBaseHelpers,string Flag) : base("VoucherGridInfo_PAY.xml", moduleName, voucherBaseHelpers)
		{
		...
		protected override void Document_Confirm_Handle(object sender, DocumentHandleEventArgs e)
		{
			...
			else
			{
                ...
                #region wenyue add for Eunice bank transfer to bank, because she dose not want to user Jouronal , Jul 2016
                for (int j = 0; j < piv1.Rows.Count; j++)
                {
                    string sqlCheckAcc = "select * from acm where accnum ='" + piv1.Rows[j]["accnum"] + "' and acctype=6";
                    ...
                    if (dsCheckAcc.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (dr_Csh.RowState != DataRowState.Deleted)
                        {
                            dr_Csh["refnum"] = payt["refnum"];
                            dr_Csh["apnum"] = payt["apnum"];
                            dr_Csh["accnum"] = piv1.Rows[j]["accnum"];
                            dr_Csh["oricur"] = piv1.Rows[j]["oricur"];
                            dr_Csh["exrate"] = piv1.Rows[j]["exrate"];
                            dr_Csh["trandate"] = piv1.Rows[j]["trandate"];
                            dr_Csh["detail"] = payt["remark"];
                            dr_Csh["docunum"] = piv1.Rows[j]["docunum"];
                            dr_Csh["remark"] = piv1.Rows[j]["remark"];
                            dr_Csh["lgr"] = "CSH";
                            dr_Csh["flag"] = "MPAY";
                            dr_Csh["status"] = "O";
                            dr_Csh["user"] = payt["user"];
                            dr_Csh["oricredit"] = (decimal)piv1.Rows[j]["oriamt"] * -1;
                            dr_Csh["oridebit"] = 0;
                            dr_Csh["oriamt"] = piv1.Rows[j]["oriamt"];
                            dr_Csh["postamt"] = piv1.Rows[j]["oriamt"];
                            dr_Csh["bankamt"] = piv1.Rows[j]["oriamt"];
                            dr_Csh["exramt"] = 0;
                            dr_Csh["period"] = BizAccounts.GetPeriod(dbaccess, (DateTime)piv1.Rows[j]["trandate"]);
                        }
                        this.dbaccess.DataSet.Tables["csh"].Rows.Add(dr_Csh);
                    }
                }
		...
		protected override void Document_Form_OnLoad(object sender, DocumentEventArgs e)
		{
            ...
            if (BizFunctions.IsEmpty(payt["accnum1"]))
            {
                payt["accnum1"] = "13003";
            }

            //if (payt["flag"].ToString().Trim().ToUpper() != "MPAY")
            //{
            //    this.formOthers = (e.FormsCollection["others"] as Form).Name;
            //}
            //this.formExcsh = (e.FormsCollection["ExCsh"] as Form).Name;
            dgOthers = BizXmlReader.CurrentInstance.GetControl(this.formdDetailName, "dg_Others") as DataGrid;
			...
			switch(e.Column.ColumnName)
			{
				case "apnum":
					...
					if (dbaccess.DataSet.Tables["getApmInfo"].Rows.Count > 0)
					{
                        ...
                        #endregion - San_End
                        if (e.Row["address"].ToString().Trim() == "" || e.Row["address"] == System.DBNull.Value)

						e.Row["address"] = getApmInfo["address"];
                    if (e.Row["phone"].ToString().Trim() == "" || e.Row["phone"] == System.DBNull.Value)

						e.Row["phone"] = getApmInfo["phone"];
                    if (e.Row["hp"].ToString().Trim() == "" || e.Row["hp"] == System.DBNull.Value)

						e.Row["hp"] = getApmInfo["hp"];
                    if (e.Row["fax"].ToString().Trim() == "" || e.Row["fax"] == System.DBNull.Value)
				...
				case "gstgrpnum":
					...
					if(this.dbaccess.DataSet.Tables["gstm"].Rows.Count != 0)
					{
                        ...
                        {
                            if (BizFunctions.IsEmpty(payt["trandate"]))
                                payt["trandate"] = DateTime.Today;
		...
		private void AutoCalc()
		{
			...
			ApTotal = apd_postamt;
			Piv1Total = piv1_postamt+piv1_gstamt;
            ...
            if (payt["status"].ToString() != Common.DEFAULT_DOCUMENT_STATUSP) // if confim only post into ard ,csh and gld
            {
                ...
                steDiff = ApTotal + Piv1Total - CshPaid;
                checkttl = (decimal)payt["oriamt"] - (decimal)payt["amtpaid"];
                txtPayStatus.Text = "AP Rounding = " + steDiff.ToString().Trim();
                if (checkttl != 0)
                {
