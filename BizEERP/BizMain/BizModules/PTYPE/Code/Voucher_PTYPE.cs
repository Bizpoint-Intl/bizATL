...
namespace ATL.PTYPE
{
	public class Voucher_PTYPE : BizRAD.BizApplication.VoucherBaseHelper
	{
		protected bool opened = false;
		protected DataRow ptype;


		public Voucher_PTYPE(string moduleName, Hashtable voucherBaseHelpers)
			: base("VoucherGridInfo_PTYPE.xml", moduleName, voucherBaseHelpers)
		{
		}
		...
		protected override void AddVoucherSelectClause(VoucherSelectEventArgs e)
		{
			...
			e.OrderClause = "active desc,ptypecode";
		}
		...
		protected override void Document_Form_OnLoad(object sender, DocumentEventArgs e)
		{
			...
			this.ptype = e.DBAccess.DataSet.Tables["ptypem"].Rows[0];
			if (this.ptype["status"].ToString().Trim() == Common.DEFAULT_DOCUMENT_STATUSN)
			{
				this.ptype["active"] = true;		//set default to Active!
			}
		}
		...
		protected override void Document_Save_Handle(object sender, DocumentHandleEventArgs e)
		{
			...
			if (BizFunctions.IsEmpty(this.ptype["ptypename"]))
			{
				MessageBox.Show("Empty Field 'Type Description' !", "Save Unsuccessful", MessageBoxButtons.OK, MessageBoxIcon.Error);
				e.Handle = false;
			}
		}
