...
namespace ATL.SCR
{
	public class Voucher_SCR : BizRAD.BizApplication.VoucherBaseHelper
	{
		...
		public Voucher_SCR(string moduleName, Hashtable voucherBaseHelpers) : base("VoucherGridInfo_SCR.xml", moduleName, voucherBaseHelpers)
		{
		...
		protected override void AddVoucherAllCondition(VoucherConditionEventArgs e)
		{
			...
			e.Condition = "sivh.flag='"+flag+"' AND sivh.systemyear = " + Common.DEFAULT_SYSTEM_YEAR;
		}
		protected override void AddVoucherDefaultCondition(VoucherConditionEventArgs e)
		{
			...
			e.Condition = " (sivh.status= '" + Common.DEFAULT_DOCUMENT_STATUSN + "' or " +
				...
				" AND sivh.flag='"+flag+"' AND sivh.systemyear = " + Common.DEFAULT_SYSTEM_YEAR;
		}
		#endregion
            ...
            if (siv1.Rows.Count > 0)
            {
                foreach (DataRow dr1 in siv1.Rows)
                {
                    if (dr1.RowState != DataRowState.Deleted)
                    {
                        ...
                        if (BizFunctions.IsEmpty(dr1["sitenum"]) && !BizFunctions.IsEmpty(sivh["arnum"]))
                        {
                            dr1["sitenum"] = BizLogicTools.Tools.GetSitenumFromDoc(sivh["arnum"].ToString(), this.dbaccess);
                        }
		...
		protected override void Document_Reopen_Handle(object sender, DocumentHandleEventArgs e)
		{
            ...
            //    " AND refnum <>'" + sivh["refnum"].ToString().Trim() + "' and flag<>'SIV' and flag<>'SDB'");

            dbaccess.ReadSQL("checkArdReopen", "SELECT refnum FROM ard" + Common.DEFAULT_SYSTEM_YEAR +
                " WHERE ISNULL(invnum,'') in (SELECT ISNULL(invnum,'') AS invnum " +
                " FROM siv1 WHERE refnum = '" + sivh["refnum"].ToString().Trim() + "') " +
                " AND refnum <>'" + sivh["refnum"].ToString().Trim() + "' and flag<>'SIV' and flag<>'SDB' "+ 
            "AND ISNULL([status],'')<>'" + Common.DEFAULT_DOCUMENT_STATUSV + "'");

			DataTable checkArdReopen = dbaccess.DataSet.Tables["checkArdReopen"];
			string RefList = "Please check entries below which has been created for this Sales Credit Note:";
            ...
            //if (RefList != "Please check entries below which has been created for this Sales Credit Note:")
            //{
			...
			if (sivh["flag"].ToString().Trim() != "SCR")
            //else if (sivh["flag"].ToString().Trim() != "SCR")
		...
		{          
            ...
            if (BizFunctions.IsEmpty(sivh["docunum"]) || sivh["docunum"].ToString().Trim() == string.Empty)
            {
                if (sivh["creditType"].ToString().Trim() == "LD" || sivh["creditType"].ToString().Trim() == "CN")
                {
                    ...
                    sivh["docunum"] = sivh["refnum"].ToString().Replace("SCR-", sivh["creditType"].ToString().Trim()).Trim();
                }
                else
                {
        ...
        protected override void Document_Extract_Handle(object sender, DocumentHandleEventArgs e)
        {
			...
			Hashtable HsExtract = MDTReader.GetExtraction("scr", "SIV-SCR Extract", TabDetail, this.dbaccess);
		...
		protected override void Document_Confirm_Handle(object sender, DocumentHandleEventArgs e)
		{
			...
			else
			{
				...
				MDTReader.updateAccount(ref this.dbaccess, "refnum|trandate|status|" +
					...
					"SIVH", "SCR", "ard", "siv10", "SIV-SCR EXTRACT");
        ...
        protected override void Document_Print_OnClick(object sender, DocumentPrintEventArgs e)
		{
            ...
            switch (e.ReportName)
            {
                case "Sales Credit Note":
					...
					//selectedCollection.Add("hemph", "SELECT * FROM hemph where empnum = '" + sivh["salesman"].ToString().Trim() + "'");
					e.DBAccess.ReadSQL(selectedCollection);
					e.DataSource = e.DBAccess.DataSet;            
                    break;


                case "Sales Credit Note2":
                    selectedCollection.Add("coy", "SELECT * FROM coy");
                    if (e.DBAccess.DataSet.Tables.Contains("arm"))
                        e.DBAccess.DataSet.Tables.Remove("arm");
                    selectedCollection.Add("arm", "SELECT top 1 * FROM arm where arnum='" + sivh["arnum"].ToString() + "'");
		...
		protected override void AddDocumentF2Condition(object sender, DocumentF2EventArgs e)
		{
			...
			switch (e.ControlName)
			{
				case "sivh_invnum":
                    ...
                    e.Condition = BizFunctions.F2Condition("sivh.refnum", (sender as TextBox).Text);
                    e.DefaultCondition = "sivh.status = 'P' and sivh.arnum = '" + sivh["arnum"].ToString().Trim() + "'";
					break;

                case "sivh_projectid":
                    //e.Condition = BizFunctions.F2Condition("sivhView.refnum", (sender as TextBox).Text);
                    //e.DefaultCondition = "sivhView.status = 'P' and sivhView.arnum = '" + sivh["arnum"].ToString().Trim() + "'";
                    //string test = " viewprojid.projectid like '" + sivh["arnum"].ToString().Substring(0, 2) + "%'";
                    e.Condition = BizFunctions.F2Condition("viewprojid.projectid", sivh["arnum"].ToString().Substring(0, 2) );
                    //e.DefaultCondition = " viewprojid.projectid like '" + sivh["arnum"].ToString().Substring(0, 2) + "%'";
                    break;
			}
		}
		...
		protected override void AddDocumentF3Condition(object sender, DocumentF3EventArgs e)
		{
			...
			switch (e.ControlName)
			{
				case "sivh_invnum":
					...
					if (!invnum.Equals(""))
					{
						string selectSivh = "SELECT * FROM sivh WHERE docunum ='" + sivh["invnum"].ToString().Trim() + "'";
						...
						if (sivhTmp.Select().Length > 0)
						{
                            ...
                            sivh["projectid"] = sivhTmp.Rows[0]["projectid"];
					...
					#region Steph - Copy the detail page
						...
						if (sivhTmp.Rows.Count != 0)
						{
                            ...
                            // 02/07/2015: Jason 
                            //string selectSiv1 = "SELECT invnum,matnum,uom,discamt,detail,sum(qty) as qty,price,line,ccnum,projectid,sitenum,accnum from siv1" +
                            //    " WHERE invnum = '" + sivh["invnum"].ToString().Trim() + "'" +
                            //    "  GROUP BY invnum,matnum,uom,price,discamt,detail,line,ccnum,projectid,sitenum,accnum order by line";

                            string selectSiv1 = "SELECT docunum as invnum,matnum,uom,discamt,detail,sum(qty) as qty,price,line,ccnum,projectid,sitenum,accnum from siv1" +
                                " WHERE docunum = '" + sivh["invnum"].ToString().Trim() + "'" +
                                "  GROUP BY docunum,matnum,uom,price,discamt,detail,line,ccnum,projectid,sitenum,accnum order by line";
                ...
                case "sivh_arnum":
                    ...
                    if (!BizFunctions.IsEmpty(e.CurrentRow["arnum"]))
                    {
                        ...
                        e.CurrentRow["accnum"] = e.F2CurrentRow["accnum"].ToString().Trim();

                        e.CurrentRow["projectid"] = BizLogicTools.Tools.GetProjectID(e.CurrentRow["arnum"].ToString(), this.dbaccess);
		...
		private void AutoCalc()
		{
			...
			if (sivh["refnum"].ToString().Trim().Contains("SCR"))
			{
            ...
            dbaccess.ReadSQL("getArmInfoAll", "Select arnum,arname,ptc,addr1,addr2,addr3,addr4,phone,hp,fax,ptnum,gstgrpnum,oricur,accnum FROM arm WHERE arnum = '"+sivh["arnum"]+"' ");
            if (dbaccess.DataSet.Tables["getArmInfoAll"].Rows.Count > 0)
            {

				DataRow getArmInfoAll = dbaccess.DataSet.Tables["getArmInfoAll"].Rows[0];
						sivh["detail"] = getArmInfoAll["arname"];

                        //sivh["addr1"] = getArmInfoAll["addr1"];
                        //sivh["addr2"] = getArmInfoAll["addr2"];
                        //sivh["addr3"] = getArmInfoAll["addr3"];
                        //sivh["addr4"] = getArmInfoAll["addr4"];
                        //sivh["phone"] = getArmInfoAll["phone"];
                        //sivh["hp"] = getArmInfoAll["hp"];
                        //sivh["fax"] = getArmInfoAll["fax"];

						if (sivh["payterms"].ToString().Trim() == "" || sivh["payterms"] == System.DBNull.Value)
							sivh["payterms"] = getArmInfoAll["ptnum"];
						if (sivh["gstgrpnum"].ToString().Trim() == "" || sivh["gstgrpnum"] == System.DBNull.Value)
							sivh["gstgrpnum"] = getArmInfoAll["gstgrpnum"];
						if (sivh["oricur"].ToString().Trim() == "" || sivh["oricur"] == System.DBNull.Value)
							sivh["oricur"] = getArmInfoAll["oricur"];
						if (sivh["accnum"].ToString().Trim() == String.Empty || sivh["accnum"] == System.DBNull.Value)
							sivh["accnum"] = getArmInfoAll["accnum"]; 
                        //if (sivh["contact"].ToString() == String.Empty || sivh["contact"] == System.DBNull.Value)
                        //    //sivh["contact"] = dbaccess.DataSet.Tables["getContactInfo"].Rows[0]["ptc"];
                        //    sivh["contact"] =getArmInfoAll["ptc"];
            }
			...
			if ((bool)sivh["inclgst"])
			{
				...
				foreach (DataRow dr in siv1.Rows)
				{
					if (dr.RowState != DataRowState.Deleted) // Do this step for those row that is not deleted
					{
						...
						BizFunctions.UpdateDataRow(sivh, dr, "refnum/arnum/docunum/invnum/flag/trandate/period/status/user/modified/oricur/exrate/gstgrpnum");
                        ...
                        if (dr["gstper"] == System.DBNull.Value || dr["gstper"].ToString().Trim() == String.Empty)
                        {

                            dr["gstper"] = sivh["gstper"];
			...
			else
			{
				...
				foreach (DataRow dr in siv1.Rows)
				{
					if (dr.RowState != DataRowState.Deleted) // Do this step for those row that is not deleted
					{
						...
						#region Steph - Adding of hscode and country of origin: 101109_1
						//reading hscode from pcatm, instead of matm_24Nov2009_0945
						//dbaccess.ReadSQL("getHsCountry", "SELECT hscode, oricountry FROM matm WHERE matnum = '" + dr["matnum"].ToString().Trim() + "'");
						dbaccess.ReadSQL("getHsCountry", "SELECT pcatm.hscode AS hscode, matm.oricountry AS oricountry FROM matm " +
							" LEFT OUTER JOIN pcatm ON matm.pcatcode = pcatm.pcatcode " +
							" WHERE matm.matnum = '" + dr["matnum"].ToString().Trim() + "'");
			...
			MDTReader.SetCorrectValue(ref this.dbaccess, "SIV1", "SCR");
		}
		#endregion
