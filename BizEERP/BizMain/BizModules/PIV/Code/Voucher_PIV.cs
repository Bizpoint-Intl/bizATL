...
namespace ATL.PIV
{
	public class Voucher_PIV : BizRAD.BizApplication.VoucherBaseHelper
	{
		...
		protected override void AddVoucherAllCondition(VoucherConditionEventArgs e)
		{
			...
			e.Condition = "pivh.flag='"+flag+"' AND pivh.systemyear = " + Common.DEFAULT_SYSTEM_YEAR;
		}
		protected override void AddVoucherDefaultCondition(VoucherConditionEventArgs e)
		{
			...
			e.Condition = " (pivh.status= '" + Common.DEFAULT_DOCUMENT_STATUSN + "' or " +
                ...
                " AND pivh.flag='" + flag + "' AND pivh.systemyear = " + Common.DEFAULT_SYSTEM_YEAR;
		}
		#endregion
		...
		protected override void Document_Save_Handle(object sender, DocumentHandleEventArgs e)
		{
            ...
            if(BizFunctions.IsEmpty(pivh["oricur"]))
            {
                pivh["oricur"] = "SGD";
            }
            ...
            if (BizFunctions.IsEmpty(pivh["accnum"]))
            {
                pivh["accnum"] = "21010";
            }
		}
		...
		protected override void AddDocumentF3Condition(object sender, DocumentF3EventArgs e)
		{
			...
			switch (e.ControlName)
			{
				case "pivh_grnum":
                    ...
                    //            " UNION ALL " +
                    //            " SELECT grnum as refnum,matnum,-qty as pqty,uom,discamt,price,detail FROM piv1) GrnVsPiv " +
                    //            " GROUP BY refnum,matnum,uom,discamt,price,detail " +
                    //            " HAVING SUM(pqty) > 0)result) AND refnum = '" + pivh["grnum"].ToString().Trim() + "'";
                ...
                case "pivh_apnum":
                    ...
                    //e.CurrentRow["accnum1"] = BizLogicTools.Tools.GetApnumAccnum(pivh["apnum"].ToString().Trim(), this.dbaccess);
                    break;
			}
		}
        ...
        void btn_ExtractGRN_Click(object sender, EventArgs e)
        {
            ...
            if (pivh["apnum"].ToString() != string.Empty)
            {
                ...
                try
                {
                    ...
                    ExtractGRN ExtractGRN = new ExtractGRN(this.dbaccess, oriTable);
                    ExtractGRN.ShowDialog();
                    ExtractGRN.Focus();

                    #region assign line number for piv1
                    int line = 100;
        ...
        void btn_Extract_Mat_Click(object sender, EventArgs e)
        {
            ...
            if (!BizFunctions.IsEmpty(pivh["dtrandateFrom"]) && !BizFunctions.IsEmpty(pivh["dtrandateTo"]))
            {
                Form frm = BizXmlReader.CurrentInstance.GetForm("Header") as Form;
                DataTable oriTable = piv1;
                try
                {
                    ExtractMIVitems2 ExtractMIV = new ExtractMIVitems2(this.dbaccess, oriTable);
		...
		protected override void Document_SaveBegin_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            if (!BizFunctions.IsEmpty(pivh["trandate"]))
            {
                pivh["invdate"] = pivh["trandate"];
            }
		...
		private void Voucher_PIV1_ColumnChanged(object sender, DataColumnChangeEventArgs e)
		{
			switch(e.Column.ColumnName)
			{
				case "matnum":
					...
					dbaccess.ReadSQL("getMatm", "SELECT matname, puom umocode,purcAcc FROM matm where matnum ='" + e.Row["matnum"].ToString().Trim() + "'");
