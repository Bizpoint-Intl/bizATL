...
namespace ATL.WOR
{
    public class Voucher_WOR : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        protected Button btnExtract1, columnButton, columnButton1, columnButton2, locColumnButton1, btn_Gen_Rpt = null;
        ...
        protected Button Btn_Show, btnLists, btnListsPic,Btn_Email = null;
        #endregion

        #region Construct

        public Voucher_WOR(string moduleName, Hashtable voucherBaseHelpers)
        ...
        protected override void AddVoucherDefaultCondition(VoucherConditionEventArgs e)
        {
            ...
            if (flag == "PWOR")
            {
                e.Condition = "ISNULL(isSubmitted,0)=0  and flag='" + flag + "' and [status]='O' ";
            }
            else
            {
        ...
        protected override void Document_Form_OnLoad(object sender, DocumentEventArgs e)
        {
            ...
            DataTable wor1 = dbaccess.DataSet.Tables["wck1"];
            ...
            if (BizFunctions.IsEmpty(worh["assignmentemail"]))
            {
                worh["assignmentemail"] = "pestcontrol@atlmaintenance.com.sg";
            }

            if (BizFunctions.IsEmpty(worh["trandate"]))
            {
                worh["trandate"] = BizFunctions.GetStandardDateString(DateTime.Now);
            }

            if (BizFunctions.IsEmpty(worh["complaindate"]))
            {
                worh["complaindate"] = BizFunctions.GetStandardDateString(DateTime.Now);
            }

            if (BizFunctions.IsEmpty(worh["assignmentdate"]))
            {
                worh["assignmentdate"] = BizFunctions.GetStandardDateString(DateTime.Now);
            }

            if (BizFunctions.IsEmpty(worh["attendancedate"]))
            {
                worh["attendancedate"] = BizFunctions.GetStandardDateString(DateTime.Now);
            }

            Btn_Email = BizXmlReader.CurrentInstance.GetControl(headerFormName, "btn_Email") as Button;
            Btn_Email.Enabled = true;
            Btn_Email.Click += new EventHandler(Btn_Email_Click);

            btn_Gen_Rpt = BizXmlReader.CurrentInstance.GetControl(headerFormName, "btn_Gen_Rpt") as Button;
            btn_Gen_Rpt.Click += new EventHandler(btn_Gen_Rpt_Click);


            if (BizFunctions.IsEmpty(worh["empname"]))
            {
                worh["empname"] = BizLogicTools.Tools.GetEmpname(worh["empnum"].ToString(), this.dbaccess);
            }
            if (BizFunctions.IsEmpty(worh["empname2"]))
            {
                worh["empname2"] = BizLogicTools.Tools.GetEmpname(worh["empnum2"].ToString(), this.dbaccess);

            }
            if (BizFunctions.IsEmpty(worh["empname3"]))
            {
                worh["empname3"] = BizLogicTools.Tools.GetEmpname(worh["empnum3"].ToString(), this.dbaccess);
                ...
                if (MessageBox.Show("There is an existing file, do you want to overwrite it?", "Notification", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    if (wor5.Rows.Count > 0)
                    {
                        ...
                        if (count > 0)
                        {
                            ...
                            string CompleteLocation = RepositoryLocation + "\\WOR\\PWOR\\" + worh["refnum"].ToString().Trim() + "\\";
        ...
        void btn_Gen_Rpt_Click(object sender, EventArgs e)
        {
            DataRow worh = this.dbaccess.DataSet.Tables["worh"].Rows[0];
            ...
            if (!BizValidate.CheckRowState(this.dbaccess.DataSet, "worh"))
            {
                ...
                string NewTaskStr = "SELECT * FROM WOR1";
                ...
                selectedCollection.Add("P1", "SELECT p1,p2,p3,p4,p5,p6,p7,p7other,svccode,locnum as location from wor1 where refnum='" + worh["refnum"].ToString().Trim() + "' and svccode='P1'");
                selectedCollection.Add("T1", "SELECT p1,p2,p3,p4,p5,p6,p7,p7other,svccode,locnum as location from wor1 where refnum='" + worh["refnum"].ToString().Trim() + "' and svccode='T1'");
                this.dbaccess.ReadSQL(selectedCollection);
        ...
        void Btn_Email_Click(object sender, EventArgs e)
        {
            ...
            //oMailItem.Body = "Dear \n\nPlease noted that there is a new Pest Job assigned to you. \n\n Ref No. " + svr["refnum"].ToString().Trim() + " .\n\n Thank You & Regards,\n\n";
        ...
        private void Initialise()
        {
            ...
            dg_outstanding = BizXmlReader.CurrentInstance.GetControl(OutstandingForm, "dg_outstanding") as DataGrid;
               
            dg_newtask = BizXmlReader.CurrentInstance.GetControl(NewTaskForm, "dg_newtask") as DataGrid;
            dg_newtask.MouseClick += new MouseEventHandler(dg_newtask_MouseClick);
            dg_newtask.MouseDown += new MouseEventHandler(dg_newtask_MouseDown);
            dg_newtask.CurrentCellChanged += new EventHandler(dg_newtask_CurrentCellChanged);

            this.bindSrc = dg_newtask.DataSource as BindingSource;

            foreach (DataGridTableStyle dataGridTableStyle in dg_newtask.TableStyles)
            {
                foreach (DataGridColumnStyle dataGridColumnStyle in dataGridTableStyle.GridColumnStyles)
                {
                    ...
                    if (dataGridColumnStyle is BizDataGridTextBoxColumn)
                    {
                        ...
                        if (bizDataGridTextBoxColumn.MappingName == "button")
                        {
                            bizDataGridTextBoxColumn.Alignment = HorizontalAlignment.Center;
                            bizDataGridTextBoxColumn.TextBoxGrid.BackColor = Color.FromKnownColor(KnownColor.Control);
                            ...
                            columnButton.Size = new Size(75, 14);
            ...
            dg_location = BizXmlReader.CurrentInstance.GetControl(LocationFormName, "dg_location") as DataGrid;
            dg_location.MouseClick += new MouseEventHandler(dg_location_MouseClick);
            dg_location.MouseDown += new MouseEventHandler(dg_location_MouseDown);
            dg_location.CurrentCellChanged += new EventHandler(dg_location_CurrentCellChanged);

            this.bindSrc = dg_location.DataSource as BindingSource;

            foreach (DataGridTableStyle dataGridTableStyle in dg_location.TableStyles)
            {
                foreach (DataGridColumnStyle dataGridColumnStyle in dataGridTableStyle.GridColumnStyles)
                {
                    ...
                    if (dataGridColumnStyle is BizDataGridTextBoxColumn)
                    {
                        ...
                        if (bizDataGridTextBoxColumn.MappingName == "button")
                        {
                            bizDataGridTextBoxColumn.Alignment = HorizontalAlignment.Center;
                            bizDataGridTextBoxColumn.TextBoxGrid.BackColor = Color.FromKnownColor(KnownColor.Control);
                            locColumnButton1 = new Button();
                            locColumnButton1.Text = "View";
                            locColumnButton1.FlatStyle = FlatStyle.Standard;
                            locColumnButton1.BackColor = Color.FromKnownColor(KnownColor.Control);
                            locColumnButton1.Size = new Size(75, 14);
                            locColumnButton1.Click += new EventHandler(locColumnButton1_Click);

                            bizDataGridTextBoxColumn.TextBoxGrid.Controls.Add(locColumnButton1);
                        }
                    }
                }
            }  
        ...
        void btnLists_Click(object sender, EventArgs e)
        {
            ...
            if (!BizFunctions.IsEmpty(worh["docunum"]))
            {
                 ...
                 else if ((worh["docunum"].ToString().Contains("CTR") || (worh["docunum"].ToString().Contains("SVC"))))
        ...
        Document_Save_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            if (BizFunctions.IsEmpty(worh["username"]))
            {
                worh["username"] = Common.DEFAULT_SYSTEM_USERNAME;
            }
        ...
        {           
            ...
            switch (e.ControlName)
            {
                ...
                case "worh_docunum":
                    {
                        if (worh["flag"].ToString().Trim() == "PWOR")
                        {
                            ...
                            e.CurrentRow["ptc"] = e.F2CurrentRow["rep1"];
                            e.CurrentRow["ptccontact"] = e.F2CurrentRow["rep1tel"];
        ...
        protected override void AddDocumentF2Condition(object sender, DocumentF2EventArgs e)
        {
            ...
            switch (e.ControlName)
            {
                ...
                case "worh_docunum":
                    {
                        if (worh["flag"].ToString().Trim() == "PWOR")
                        {
                            e.DefaultCondition= "refnum like 'P%' ";
                        }
                        else if (worh["flag"].ToString().Trim() == "WOR")
                        {
                            e.DefaultCondition = "refnum like 'CTR%' ";
                        }
                    }
                    break;
                ...
                case "worh_sitenum":
                    {
                        if (worh["flag"].ToString().Trim() == "PWOR")
                        {
                            e.DefaultCondition = "sitenum in (Select sitenum from ctrh where refnum like 'P%') ";
                        }
                        else if (worh["flag"].ToString().Trim() == "WOR")
                        {
                            e.DefaultCondition = "refnum like 'CTR%' ";
                ...
                case "worh_assignmentname":
                    {
                        if (worh["flag"].ToString().Trim() == "PWOR")
                        {
                            e.DefaultCondition = "sitenum='PCT' AND  (statuscode NOT LIKE 'RES%' AND statuscode NOT LIKE 'CEA%') ";
                        }
                    }
                    break;
        ...
        protected override void Document_Refresh_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            if (wor1.Rows.Count > 0)
            {
                foreach (DataRow dr1 in wor1.Rows)
                {
                    if (dr1.RowState != DataRowState.Deleted)
                    {
                        if (BizFunctions.IsEmpty(dr1["zempnum"]) && !BizFunctions.IsEmpty(dr1["sitenum"]))
                        {
                            dr1["zempnum"] = getZoneExec(dr1["sitenum"].ToString().Trim());
                        }
                    }
                }
            }
            ...
            if(wor4.Rows.Count > 0)
            {
                foreach(DataRow dr4 in wor4.Rows)
                {
                    if(dr4.RowState != DataRowState.Deleted)
                    {
                        if(!BizFunctions.IsEmpty(dr4["signoffdt"]))
                        {
                            dr4["worstatus"] = "COMPLETED";
                        }
                    }
                }
            }
        ...
        private void GetByOE()
        {
                        ...
                        for (int i = 1; i <= 12; i++)
                        {
                            if (Convert.ToDateTime(worh["worDateFrom"]).Month == i)
                            {
                                if (!BizFunctions.IsEmpty(dr6["xmonth" + i.ToString() + ""]) && !BizFunctions.IsEmpty(dr6["year"]))
                                {
                                    ...
                                    UpdateScheduleCTR(dr6["svccode"].ToString(), dr6["svcdesc"].ToString(), dr6["location"].ToString(), dr6["frequencycode"].ToString(), i.ToString(), dr6["year"].ToString(), Convert.ToInt32(Convert.ToBoolean(dr6["xmonth" + i.ToString() + ""])).ToString());
                                }
                            }
                        }
                    }
            ...
            if (SITMTPD_TBtmp.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr1 in SITMTPD_TBtmp.Rows)
                {
                    ...
                    InsertWor1["svcdesc"] = dr1["svcdesc"];
                        ...
                        for (int i = 1; i <= 12; i++)
                        {
                            if (Convert.ToDateTime(worh["worDateFrom"]).Month == i)
                            {
                                if (!BizFunctions.IsEmpty(dr6["xmonth" + i.ToString() + ""]) && !BizFunctions.IsEmpty(dr6["year"]))
                                {
                                    ...
                                    UpdateScheduleADH(dr6["svccode"].ToString(), dr6["svcdesc"].ToString(), dr6["location"].ToString(), dr6["frequencycode"].ToString(), i.ToString(), dr6["year"].ToString(), Convert.ToInt32(Convert.ToBoolean(dr6["xmonth" + i.ToString() + ""])).ToString());
                                }
                            }
                        }
                    }
        ...
        private void GetByCustomer()
        {
            ...
            string strGetAhPd = "SELECT " +
                                    ...
                                    "S11.[GUID] " +
                                "FROM " +
                                "(select sitenumt,frequencycode,svccode,svcdesc,location,[guid],CONVERT(DATETIME,CONVERT(nvarchar,dayno)+'/'+ CONVERT(nvarchar,xmonth)+'/'+CONVERT(nvarchar,[year]),103) as pdate from SITMT11 WHERE [status]<>'V')S11 " +
                                "LEFT JOIN SITMT ST ON S11.sitenumt = ST.sitenumt " +
                                    ...
                                    "SELECT [guid] FROM " +
                                      "( "+
                                         "SELECT " +
                                            ...
                                            "S11.[GUID] " +
                                                "FROM " +
                                                "(select sitenumt,frequencycode,svccode,svcdesc,[guid],CONVERT(DATETIME,CONVERT(nvarchar,dayno)+'/'+ CONVERT(nvarchar,xmonth)+'/'+CONVERT(nvarchar,[year]),103) as pdate from SITMT11 WHERE [status]<>'V')S11 " +
                                                "LEFT JOIN SITMT ST ON S11.sitenumt = ST.sitenumt " +
                                                "where ST.docunum not like 'ADH%' AND ST.[status]<>'V' AND CONVERT(nvarchar,S11.pdate,112)>='" + BizFunctions.GetSafeDateString(Convert.ToDateTime(worh["worDateFrom"])) + "' and CONVERT(nvarchar,S11.pdate,112)<='" + BizFunctions.GetSafeDateString(Convert.ToDateTime(worh["worDateTo"])) + "' and arnum='" + worh["arnum"].ToString() + "' " +
                                                "and S11.[guid] in (Select origuid from wor1 where [status]<>'V')" +
        ...
        //{
        //    DataTable SigTB = BizFunctions.ExecuteQuery(this.dbaccess.DataSet, "select refnum,ctrnum,sitenum,servicerptloc from wor4");

        //    SigTB.TableName = "SigTB";

        //    SigTB.Columns.Add("Photo", typeof(Byte[]));
        ...
        //            if (dr1.RowState != DataRowState.Deleted)
        //            {
        //                if (!BizFunctions.IsEmpty(dr1["servicerptloc"]))
        //                {
        //                    dr1["photo"] = System.IO.File.ReadAllBytes(dr1["servicerptloc"].ToString().Trim());
        //                }
        ...
        //Added svcdesc: Jason 30-07-2015
        ...
        private void UpdateScheduleCTR(string svcCode,string svcDesc, string location, string frequencyCode, string month, string year, string days)
        {
            int totalDays = 0;
            string[] sTemp = days.Split(',');
            DateTime dt = new DateTime(Convert.ToInt32(year), Convert.ToInt32(month), 1);
            ...
            if (days != string.Empty)
            {
                if (days != "0")
                {
                    if (!days.Contains("99"))
                    {
                        ...
                        for (int i = 0; i < Convert.ToInt32(days); i++)
                        {
                            InsertSchedule(svcCode,svcDesc, location, frequencyCode, month, Convert.ToString(1), year);
                        }
                    }
                    else
                    {
        ...
        private void UpdateScheduleADH(string svcCode, string svcDesc, string location, string frequencyCode, string month, string year, string days)
        {
            int totalDays = 0;
            string[] sTemp = days.Split(',');
            DateTime dt = new DateTime(Convert.ToInt32(year), Convert.ToInt32(month), 1);
        ...
        private void InsertSchedule(string svcCode,string svcDesc, string location, string frequencyCode, string xMonth, string dayNo, string year)
        {
            DataRow WORH = this.dbaccess.DataSet.Tables["WORH"].Rows[0];
