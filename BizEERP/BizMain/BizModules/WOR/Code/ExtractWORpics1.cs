...
{
    public partial class ExtractWORpics1 : Form
    {
        protected DBAccess dbaccess = null;
        protected string Reference, Guid = "";


        public ExtractWORpics1(DBAccess da, string refnum)
        {
            InitializeComponent();
            this.dbaccess = da;
        ...
        private void ExtractWORpics1_Load(object sender, EventArgs e)
        {
            if (!HasData())
            {
        ...
        {
            bool hasData = false;
            string getPics = "Select ID,photourl,remark,created from WOR5 where refnum='" + Reference + "' ";
            ...
            if (getPicsTB != null)
            {
                if (getPicsTB.Rows.Count > 0)
                {
                    ...
                    {
                        if (BizFunctions.IsEmpty(getPicsTB.Rows[i]["created"]))
                        {
                            getPicsTB.Rows[i]["created"] = this.dbaccess.DataSet.Tables["worh"].Rows[0]["trandate"];
                        }
