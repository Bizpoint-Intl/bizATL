			...
			switch (e.ControlName)
			{
				case "sivh_invnum":
					...
					if (!invnum.Equals(""))
					{
						...
						if (sivhTmp.Rows.Count != 0)
						{
							...
							string selectSiv1 = "SELECT invnum,matnum,uom,discamt,detail,sum(qty) as qty,price,ccnum,projectid,sitenum from siv1" +
                ...
                case "sivh_arnum":
                    ...
                    if (!BizFunctions.IsEmpty(e.CurrentRow["arnum"]))
                    {
                        ...
                        e.CurrentRow["accnum"] = e.F2CurrentRow["accnum2"].ToString();
            ...
            if (siv1.Rows.Count > 0)
            {
                foreach (DataRow dr1 in siv1.Rows)
                {
                    if (dr1.RowState != DataRowState.Deleted)
                    {
                        ...
                        {
                            dr1["sitenum"] = BizLogicTools.Tools.GetSitenumFromDoc(dr1["arnum"].ToString(), this.dbaccess);
                        }
