...
namespace ATL.TRQ
{
    public class Voucher_TRQ : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        protected override void Document_Form_OnLoad(object sender, DocumentEventArgs e)
        {
            ...
            {
                trqh["senddate"] = BizFunctions.GetStandardDateString((DateTime)System.DateTime.Now.Date);
        ...
        private decimal previousQtyRequest(string matnum)
        {
            ...
            string getPreviousQty = "select (t1.matnum) , qty as previousQtyRequest from trq1  t1 where t1.fromsitenum='" + trqh["fromsitenum"] + "' " +
" and flag is not null and t1.matnum='" + matnum + "'  and refnum<>'" + trqh["refnum"] + "' and trandate<'" + BizFunctions.GetSafeDateString((DateTime)trqh["trandate"]) + "' order by trandate";
        ...
        private DateTime previousQtyRequest(string matnum,string bb)
        {
            DateTime previousdate=DateTime.Today.Date ;
            DataRow trqh = this.dbaccess.DataSet.Tables["trqh"].Rows[0];
...
" and flag is not null and t1.matnum='" + matnum + "'  and refnum<>'" + trqh["refnum"] + "' and trandate<'" + BizFunctions.GetSafeDateString((DateTime)trqh["trandate"]) + "' order by trandate";
        ...
        protected override void Document_Save_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            if (e.Handle)
            {
                if (BizFunctions.IsEmpty(trqh["sitenum"]))
                {
                    //string command = "select sitenum from posm where posnum = '" + trqh["fromsitenum"].ToString() + "'";
                    //e.DBAccess.ReadSQL("posm", command);
                    //DataRow posm = e.DBAccess.DataSet.Tables["posm"].Rows[0];
                    //trqh["sitenum"] = posm["sitenum"].ToString();
                    //posid = posm["sitenum"].ToString();
                ...
                foreach (DataRow dr1 in trq1.Rows)
                {
                    if (dr1.RowState != DataRowState.Deleted)
                    {
                        if (!BizFunctions.IsEmpty(dr1["matnum"]))
                        {
                            ...
                            dr1["senddate"] = previousQtyRequest(dr1["matnum"].ToString().Trim(),"");
        ...
        protected override void Document_SaveBegin_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            //        if (dr.RowState != DataRowState.Deleted)
            //        {
            //            //Copies some of the trqh's column data to trq1
            //            BizFunctions.UpdateDataRow(trqh, dr, "fromsitenum,tositenum,sentby,sitenum,senddate,trandate,year,flag,status,confirms,user,modified");
            //        }
            //    }
            //}
            # endregion
        ...
        protected override void Document_SaveEnd_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            if (trqh["status"].ToString().Trim() == Common.DEFAULT_DOCUMENT_STATUSP)
            {
                ...
                //        #region Save to trqh,trq1 to backend
                ...
                //                int id = 0;
                //                DataSet trqhmaxid = remoteDBAccess.ReadSQLTemp("trqid", "Select max(id) as 'id' from " + dataTable.TableName);

                //                if (trqhmaxid.Tables["trqid"].Rows.Count > 0)
                //                {
                //                    if (trqhmaxid.Tables["trqid"].Rows[0]["id"] == System.DBNull.Value)
                //                        id = 0;
                //                    else
                //                        id = Convert.ToInt32(trqhmaxid.Tables["trqid"].Rows[0]["id"]) + 1;
                //                }
                //                else
                //                {
                //                    id = 0;
                ...
                //        remoteDBAccess.RemoteStandardSQL.ExecuteNonQuery("DELETE FROM trqh WHERE refnum = '" + trqh["refnum"].ToString().Trim() + "'");
                //        remoteDBAccess.RemoteStandardSQL.ExecuteNonQuery("DELETE FROM trq1 WHERE refnum = '" + trqh["refnum"].ToString().Trim() + "'");

                //        remoteDBAccess.Update(dataTables); 
                //        remoteDBAccess.RemoteStandardSQL.ExecuteNonQuery("UPDATE sysid set lastid = (Select max(id) from trqh) where tablename = 'TRQH'");
                //        remoteDBAccess.RemoteStandardSQL.ExecuteNonQuery("UPDATE sysid set lastid = (Select max(id) from trq1) where tablename = 'TRQ1'");
                ...
                //    DataRow trih = dbaccess.DataSet.Tables["trqh"].Rows[0];

                //    #region Front End Status Roll Back due to BackEnd Saving Failure
                //    //update the status of backend trqh, trq1 to 'O' and update trqh(trinum) to ' '
                //    string updateString0 =
                //    "Update trqh set status='" + Common.DEFAULT_DOCUMENT_STATUSO + "' where refnum ='" + trqh["refnum"].ToString().Trim() + "'";
                //    dbaccess.RemoteStandardSQL.ExecuteNonQuery(updateString0);
                //    string updateString1 =
                //    "Update trq1 set status='" + Common.DEFAULT_DOCUMENT_STATUSO + "' where refnum ='" + trqh["refnum"].ToString().Trim() + "'";
