...
{
    public partial class ATRFilter : Form
    {
        string projectPath = "";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string EmployeeListStr = "SELECT DISTINCT EMPNUM FROM	"+	
		                            "( "+	
				                             "SELECT "+
                                                    "  '<center> <p>'+  ISNULL(C.shiftcode,'')+' </p> </center>  <center> <p>'+ISNULL(CONVERT(nvarchar(8),C.OTHrs),'0')+'/'+ISNULL(CONVERT(nvarchar(8),C.lateHrs),'0')+'/'+ISNULL(CONVERT(nvarchar(8),C.DailyAllow),'0')+' </p>  </center>   <center> <p>'+ isnull((select lvmnum+'('+Status+')' from lvr where c.empnum=lvr.empnum and c.Date >= lvr.leavefrom and c.Date <= leaveto ),'-') +' </p> </center> '   as detail,   " +
					                             ...
					                             "from ATMRLIVE al "+
						                             "LEFT JOIN HEMPH HP on al.empnum=HP.empnum "+
							                             "LEFT JOIN SITM SM on al.sitenum=SM.sitenum "+
                                                             ...
                                                             "WHERE MONTH(al.[Date])=" + monthComboBox1.Text.Trim() + " and YEAR(al.[Date])=" + yearComboBox1.Text.Trim() + " and al.sitenum='" + siteTextBox.Text.Trim() + "' and al.shiftcode<>'V' " + additionalWhereClause + "  " +
                                                 "GROUP BY al.empnum,HP.empname,al.[Date],al.sitenum,SM.sitename,al.sitenumt,al.shiftcode,al.[timein],al.[timeout],al.ActualDateTimeIn,al.ActualDateTimeOut " +
            ...
            //                                             "LEFT JOIN SITM SM on al.sitenum=SM.sitenum " +
            //                                                 "LEFT JOIN vSHLV vS on al.shiftcode=vS.SHIFTCODE " +
            //                                     "WHERE MONTH(al.[Date])=" + monthComboBox1.Text.Trim() + " and YEAR(al.[Date])=" + yearComboBox1.Text.Trim() + "  and al.sitenum='" + siteTextBox.Text.Trim() + "' and al.shiftcode<>'V'   " +
            ...
            //Jason: 28/07/2015 - Added Clockin Mark
            string EmployeeScheduleListStr = "SELECT " +
                                                 "  '<center> <p>'+  ISNULL(C.shiftcode,'')+' </p> </center>  <center> <p>'+ISNULL(CONVERT(nvarchar(8),C.OTHrs),'0')+'/'+ISNULL(CONVERT(nvarchar(8),C.lateHrs),'0')+'/'+ISNULL(CONVERT(nvarchar(8),C.DailyAllow),'0')+' </p>  </center>   <center> <p>'+ isnull((select TOP 1 lvmnum+'('+Status+')' from lvr where c.empnum=lvr.empnum and c.Date >= lvr.leavefrom and c.Date <= leaveto AND lvr.[status]<>'V' ),'-') +' </p> </center> '   as detail,   " +
                                                 "*  " +
                                             "FROM  " +
                                             "(  " +
                                             "SELECT  " +
                                                 ...
                                                 "B.sitename, " +
                                                 "B.sitenumt, " +
                                                 // Added transfer 'T' shiftcode: 22/04/2016
                                                 "CASE WHEN ISNULL(B.isWorkShift,0)>0 and b.ClockInMark>0 and B.shiftcode<>'TRF'  THEN 'P' WHEN ISNULL(B.isWorkShift,0)>0 and b.ClockInMark=0 THEN '-' WHEN B.shiftcode='TRF' THEN 'T'  else  B.shiftcode END as shiftcode, "+
                                                 //"CASE WHEN ISNULL(B.isWorkShift,0)>0 and b.ClockInMark>0  THEN 'P' WHEN ISNULL(B.isWorkShift,0)>0 and b.ClockInMark=0 THEN '-'  else  B.shiftcode END as shiftcode, " +
                                                 "B.[timein], " +
                                                 "B.[timeout], " +
                                                 "B.ActualDateTimeIn, " +
                                                 "B.ActualDateTimeOut, " +
                                             ...
                                             "select  " +
                                                 ...
                                                 "SUM(ISNULL(al.FixAllowAmt,0)) AS DailyAllow, " +
                                                 "SUM(ISNULL(vS.isWorkShift,0)) AS isWorkShift,	 " +
                                                 " SUM(ISNULL(CONVERT(int,al.ClockInMark),0)) AS ClockInMark "+
                                                 "from ATMRLIVE al " +
                                                     "LEFT JOIN HEMPH HP on al.empnum=HP.empnum " +
                                                         "LEFT JOIN SITM SM on al.sitenum=SM.sitenum " +
                                                             "LEFT JOIN vSHLV vS on al.shiftcode=vS.SHIFTCODE " +
                                                 ...
                                                 "GROUP BY al.empnum,HP.empname,HP.datejoined,HP.dateresigned,al.[Date],al.sitenum,al.sitenumt,SM.sitename,al.shiftcode,al.[timein],al.[timeout],al.ActualDateTimeIn,al.ActualDateTimeOut " +
            ...
            DataTable EmployeeSitenumtDt = BizFunctions.ExecuteQuery(this.dbAccess.DataSet, "Select DISTINCT sitenumt from EmployeeScheduleList");
            DataTable EmployeeSitenumt = null;
            EmployeeSitenumt = EmployeeSitenumtDt.Copy();
            EmployeeSitenumt.TableName = "EmployeeSitenumt";

            if (this.dbAccess.DataSet.Tables.Contains("EmployeeSitenumt"))
            {
                this.dbAccess.DataSet.Tables["EmployeeSitenumt"].Dispose();
                this.dbAccess.DataSet.Tables.Remove("EmployeeSitenumt");

                this.dbAccess.DataSet.Tables.Add(EmployeeSitenumt);
            }
            else
            {
                this.dbAccess.DataSet.Tables.Add(EmployeeSitenumt);
            ...
            AttendanceTB1.Columns.Add("sitenumt", typeof(string));
            ...
            if (EmployeeList.Rows.Count > 0)
            {
                foreach (DataRow dr1 in EmployeeList.Rows)
                {
                    foreach (DataRow dr3 in EmployeeSitenumt.Rows)
                    {
                        DataRow insertAttendanceTB1 = AttendanceTB1.NewRow();
                        insertAttendanceTB1["empnum"] = dr1["empnum"];
                        insertAttendanceTB1["empname"] = GetEmpDetail(dr1["empnum"].ToString().Trim(), "empname");
                        insertAttendanceTB1["yearno"] = yearComboBox1.Text.Trim();
                        insertAttendanceTB1["monthno"] = monthComboBox1.Text.Trim();
                        insertAttendanceTB1["sitenumt"] = dr3["sitenumt"];
                        if (EmployeeScheduleList.Rows.Count > 0)
                        {
                            foreach (DataRow dr2 in EmployeeScheduleList.Rows)
                            {
                                if (dr1["empnum"].ToString().Trim() == dr2["empnum"].ToString().Trim() && dr2["sitenumt"].ToString().Trim() == dr3["sitenumt"].ToString().Trim())
                                {

                                    string test1 = dr1["empnum"].ToString().Trim();
                                    string test2 = dr2["detail"].ToString().Trim();
                                    string test6 = dr2["sitenumt"].ToString().Trim();
                                    string test5 = dr2["shiftcode"].ToString().Trim();
                                    ...
                                    for (int i = 1; i <= 31; i++)
                                    {
                                        ...
                                        if (i.ToString().Trim() == dr2["dayno"].ToString().Trim())
                                        {
                                            // if the day has more than lines
                                            // if at those same day there is 1 shiftcode
                                            // Mark as ShiftCode
                                            // else mark as absent
                                            // Just get the value of that 1 line

                                            //if (!isLineExistInAttendanceTB1(dr1["empnum"].ToString().Trim(), dr2["dayno"].ToString().Trim()))
                                            //{
                                            if (CountLines(dr1["empnum"].ToString().Trim(), dr2["dayno"].ToString().Trim()) > 1)
                                            {
                                                if (isPresent(dr1["empnum"].ToString().Trim(), dr2["dayno"].ToString().Trim()))
                                                {
                                                    if (dr2["shiftcode"].ToString().Trim() != "-")
                                                    {
                                                        insertAttendanceTB1[i.ToString().Trim()] = dr2["detail"];
                                            ...
                                            else if (CountLines(dr1["empnum"].ToString().Trim(), dr2["dayno"].ToString().Trim()) == 1)
                                            {
                                                insertAttendanceTB1[i.ToString().Trim()] = dr2["detail"];
                                            }
            ...
            if (allRD1.Checked == true)
            {
                ...
                //crReportDocument1.Load(this.projectPath + @"\ATRPRNT\AttendanceReport1_1_03022016.rpt");
                ...
                //crReportDocument2.Load(this.projectPath + @"\ATRPRNT\AttendanceReport1_2_03022016.rpt");
        ...
        private int CountLines(string empNum, string dayNo)
        {
            int count = 0;
            string sql1 = "Select count(*) as total from EmployeeScheduleList where empnum='"+empNum+"' and dayno='"+dayNo+"' ";

            DataTable dt = BizFunctions.ExecuteQuery(this.dbAccess.DataSet,sql1);

            if(dt != null)
            {
                if(dt.Rows.Count>0)
                {
                    if(!BizFunctions.IsEmpty(dt.Rows[0]["total"]))
                    {
        ...
        private bool isPresent(string empNum, string dayNo)
        {
            bool Present = false;
            int count = 0;
            string sql1 = "Select count(*) as total from EmployeeScheduleList where empnum='" + empNum + "' and dayno='" + dayNo + "' and shiftcode<>'-'";
        ...
        private bool isLineExistInAttendanceTB1(string empNum, string dayNo)
        {
            bool Present = false;
            int count = 0;
            string sql1 = "Select "+dayNo+" as detail from AttendanceTB1 where empnum='" + empNum + "' ";
            ...
            if (dt != null)
            {
                if (dt.Rows.Count > 0)
                {
                    if (!BizFunctions.IsEmpty(dt.Rows[0]["detail"]))
                    {
