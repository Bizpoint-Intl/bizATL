...
namespace ATL.MNM
{
    public class Voucher_MNM : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        public Voucher_MNM(string moduleName, Hashtable voucherBaseHelpers)

            : base("VoucherGridInfo_MNM.xml", moduleName, voucherBaseHelpers)
        {
        }
        #endregion Construct
        ...
        protected override void Document_Form_OnLoad(object sender, DocumentEventArgs e)
        {
            ...
            e.DBAccess.DataSet.Tables["MNM"].ColumnChanged += new DataColumnChangeEventHandler(Voucher_MNM_ColumnChanged);
        ...
        Document_Save_Handle(object sender, DocumentHandleEventArgs e)
        {
            ...
            DataRow mnm = this.dbaccess.DataSet.Tables["mnm"].Rows[0];
            UserAuthorization sa = new UserAuthorization(this.moduleName.ToString());
        

            if (e.Handle && CheckIFhasExistingModuleName())
            {
                MessageBox.Show("Module Name. '" + mnm["Modulename"].ToString() + "' has already set and can't be duplicated.", "Pico Guards Ltd. Pte.", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);

                e.Handle = false;
            }

            if (e.Handle && BizFunctions.IsEmpty(mnm["modulename"]))
            {
        ...
        protected override void Document_SaveBegin_OnClick(object sender, DocumentEventArgs e)
        {
            ...
            DataRow mnm = this.dbaccess.DataSet.Tables["MNM"].Rows[0];
            DataTable mnm1 = this.dbaccess.DataSet.Tables["mnm1"];

           

            foreach (DataRow dr1 in mnm1.Rows)
            {
                if (dr1.RowState != DataRowState.Deleted)
                {
                    BizFunctions.UpdateDataRow(mnm, dr1, "Modulecode/ModuleName/user/flag/status/created/modified");
                }
            }
        ...
        private void Voucher_MNM_ColumnChanged(object sender, DataColumnChangeEventArgs e)
        {
        ...
        private bool CheckIFhasExistingModuleName()
        {
            ...
            if (!BizFunctions.IsEmpty(mnm["modulename"]))
            {
                string str1 = "Select * from mnm where modulename='" + mnm["modulename"].ToString() + "' and modulecode<>'" + mnm["modulecode"].ToString() + "'";
                this.dbaccess.ReadSQL("dtCHECKmnm", str1);

                if (this.dbaccess.DataSet.Tables["dtCHECKmnm"].Rows.Count > 0)
                {
                    hasRecord = true;
                }
            }
            this.dbaccess.DataSet.Tables["dtCHECKmnm"].Dispose();
            return hasRecord;
        }
