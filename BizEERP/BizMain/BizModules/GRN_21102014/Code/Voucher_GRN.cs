...
namespace ATL.GRN
{
	public class Voucher_GRN : BizRAD.BizApplication.VoucherBaseHelper
	{
		...
		public Voucher_GRN(string moduleName, Hashtable voucherBaseHelpers) : base("VoucherGridInfo_GRN.xml", moduleName, voucherBaseHelpers)
		{
				...
				case "grnh_ponum":
					...
					{		
						...
						if (porhTmp.Rows.Count != 0)
						{
                            ...
                            string selectPor1 = "SELECT ponum,matnum,uom,discamt,detail,sum(qty) as qty,price from " +
                                "(SELECT refnum as ponum, matnum,uom,discamt,detail, qty,price from por1 UNION ALL" +
                                " SELECT ponum, matnum,uom,discamt,detail,-qty as qty,0 as price from grn1 WHERE refnum <>'" + grnh["refnum"].ToString().Trim() + "')a" +
                                "  WHERE ponum = '" + grnh["ponum"].ToString().Trim() + "' GROUP BY ponum,matnum,uom,discamt,detail,price" +
                                " HAVING SUM(qty) >0";
