 ...
 *	Depcription:    Purchase Credit Note Vouchers
 *	Function List:	
 * 
 * History			历史修改添加记录
 * ---------------------------------------------------------
...
namespace ATL.PCR
{
	public class Voucher_PCR : BizRAD.BizApplication.VoucherBaseHelper
	{
        ...
        protected Button btnExtractGRT = null;
        protected TextBox pivh_invnum = null;
		#endregion

		public Voucher_PCR(string moduleName, Hashtable voucherBaseHelpers) : base("VoucherGridInfo_PCR.xml", moduleName, voucherBaseHelpers)
		{
		}
		...
		protected override void Document_Confirm_Handle(object sender, DocumentHandleEventArgs e)
		{
			...
			else
			{
				#region Steph - Posting Time!!!
					...
					"accnum|apnum|exrate|detail|flag|lgr|gstamt|invdate|dpostamt|postamt|doriamt|"+
					"oriamt|oricur|period|oricredit|oridebit|invnum|gstamt|exramt|supinvnum",
					"PIVH", "PCR", "apd", "piv10", "PIV-PCR EXTRACT");
		...
		protected override void AddVoucherAllCondition(VoucherConditionEventArgs e)
		{
			...
			e.Condition = "pivh.flag='PCR' AND pivh.systemyear = " + Common.DEFAULT_SYSTEM_YEAR;
		}
		protected override void AddVoucherDefaultCondition(VoucherConditionEventArgs e)
		{
			...
			e.Condition = " (pivh.status= '" + Common.DEFAULT_DOCUMENT_STATUSN + "' or " +
				...
				" AND pivh.flag='PCR' AND pivh.systemyear = " + Common.DEFAULT_SYSTEM_YEAR;
		}		
		#endregion
		...
		protected override void AddDocumentF2Condition(object sender, DocumentF2EventArgs e)
		{
			...
			switch (e.ControlName)
			{
				case "pivh_invnum":
					...
					break;
                case "pivh_grtnum":
                    e.Condition = BizFunctions.F2Condition("grth.refnum", (sender as TextBox).Text);
                    e.DefaultCondition = "grth.status = 'P' and grth.apnum = '" + pivh["apnum"].ToString().Trim() + "'"+
                                                " AND (grth.refnum IN " +
                                                " (SELECT refnum FROM (SELECT ISNULL(refnum,'') AS refnum,matnum,sum(qty) AS qty FROM " +
                                                " (SELECT refnum,matnum,qty FROM grt1 " +
                                                " UNION ALL " +
                                                " SELECT grtnum as refnum,matnum,qty FROM piv1) tmp " +
		...
		protected override void AddDocumentF3Condition(object sender, DocumentF3EventArgs e)
		{
			...
			switch (e.ControlName)
			{
                ...
                case "pivh_grtnum":
                    #region Extraction from pivh(PIV) to pivh(SCR)
                    string grtnum = pivh["grtnum"].ToString().Trim();
                    if (!grtnum.Equals(""))
                    {
                        string selectGrth = "SELECT * FROM grth WHERE refnum ='" + pivh["grtnum"].ToString().Trim() + "'";
                        this.dbaccess.ReadSQL("grthTmp", selectGrth);
                        DataTable grthTmp = this.dbaccess.DataSet.Tables["grthTmp"];

                        if (grthTmp.Select().Length > 0)
                        {
                            pivh["apnum"] = grthTmp.Rows[0]["apnum"];
                            pivh["oricur"] = grthTmp.Rows[0]["oricur"];
                            pivh["remark"] = grthTmp.Rows[0]["remark"];
                            pivh["gstgrpnum"] = grthTmp.Rows[0]["gstgrpnum"];
                        }
                        else
                        {
                            pivh["apnum"] = "";
                    ...
                    #region Steph - Copy the detail page
                        ...
                        if (grthTmp.Rows.Count != 0)
                        {
                            #region Steph - Import grt1 to piv1

                            string selectGrt1 = "SELECT matnum,uom,discamt,detail,sum(qty) as qty,price from grt1" +
                                " WHERE refnum = '" + pivh["grtnum"].ToString().Trim() + "'" +
                                "  GROUP BY matnum,uom,price,discamt,detail";

                            this.dbaccess.ReadSQL("grt1Tmp", selectGrt1);
                            DataTable grt1Tmp = this.dbaccess.DataSet.Tables["grt1Tmp"];

                            BizFunctions.DeleteAllRows(piv1);
                            foreach (DataRow dr in grt1Tmp.Select())
                            {
                                dr.SetAdded();
                                piv1.ImportRow(dr);
                            }
		...
		protected override void Document_Form_OnLoad(object sender, DocumentEventArgs e)
		{
            ...
            btnExtractGRT = BizXmlReader.CurrentInstance.GetControl(headerFormName, "btn_ExtractGRT") as Button;
            btnExtractGRT.Click += new EventHandler(btnExtractGRT_Click);
            pivh_invnum = BizXmlReader.CurrentInstance.GetControl(headerFormName, "pivh_invnum") as TextBox;
            //pivh_invnum.Enabled = false; // Commented by Jason: Irene Requested. Don't know why it is supposed to be disabled from the very beginning
        ...
        void btnExtractGRT_Click(object sender, EventArgs e)
        {
            DataRow pivh = dbaccess.DataSet.Tables["pivh"].Rows[0];
            DataTable piv1 = dbaccess.DataSet.Tables["piv1"];
            ...
            if (pivh["apnum"].ToString() != string.Empty)
            {
                ...
                {
                    // Open Extract Form
                    ExtractGRTForm ExtractGRT = new ExtractGRTForm(this.dbaccess, oriTable);
                    ExtractGRT.ShowDialog(frm);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(frm, ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
		...
		protected override void Document_Reopen_Handle(object sender, DocumentHandleEventArgs e)
		{
			...
			if (pivh["flag"].ToString().Trim() != "PCR")
			{
				e.Handle = false;
			}
		...
		{          
			...
			MDTReader.SetCorrectValue(ref this.dbaccess, "PIV1", "PCR");
        }
        ...
        protected override void Document_Extract_Handle(object sender, DocumentHandleEventArgs e)
        {
			...
			Hashtable HsExtract = MDTReader.GetExtraction("pcr", "PIV-PCR Extract", TabDetail, this.dbaccess);
		...
		private void AutoCalc()
		{
			...
			if (pivh["refnum"].ToString().Trim().Contains("PCR"))
			{
