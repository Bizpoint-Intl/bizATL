...
namespace ATL.HEMP
{
    public class Voucher_HEMP : BizRAD.BizApplication.VoucherBaseHelper
    {
        ...
        protected Button btn_Voucher_Reports, Btn_Contract, Btn_Picture, columnButton1, columnButton2, docColumnUpButton, docColumnPrevButton = null;
        protected DataGrid SalHistorydatagrid, EmpHistorydatagrid, SalRecordDG, dg_warning, dg_appraise, dg_archive = null;
        ...
        protected override void Document_Form_OnLoad(object sender, DocumentEventArgs e)
        {
            ...
            DataGrid dg_archive = (DataGrid)BizXmlReader.CurrentInstance.GetControl(archiveFormName, "dg_archive");
                ...
                foreach (DataGridColumnStyle dataGridColumnStyle in dataGridTableStyle.GridColumnStyles)
                {
                    ...
                    if (dataGridColumnStyle is BizDataGridTextBoxColumn)
                    {
                        ...
                        if (bizDataGridTextBoxColumn.MappingName == "button2")
                        {
                            ...
                            docColumnPrevButton.Text = "Upload";
            ...
            switch (e.ReportName)
            {
                ...
                case "Application Form":
                    ...
                    string str = "Select " +
                             ...
                             "A.NextOfKeen, " +
                             "A.FamilyContact, " +
                             "ISNULL(A.[Basic],0)+ISNULL(A.ATTNALLW,0)+ISNULL(A.ACCOMALLW,0)+ISNULL(A.DRIVTRANSALLW,0)+ISNULL(A.OtherAllowance,0)+ISNULL(A.OTALLW,0) as TotalSalary " +
        ...
        {
            DataRow hemph = this.dbaccess.DataSet.Tables["HEMPH"].Rows[0];
            string sql1 = "select lastday from ERRH where empnum='" + hemph["empnum"].ToString() + "' and status='P' AND ISNULL(isapproved,0)=1";
            if (!BizFunctions.IsEmpty(hemph["empnum"]))
            {
                ...
                if (dtGetSalH1 != null)
                {
                    if (dtGetSalH1.Rows.Count > 0)
                    {
                        ...
                        for (int i = dtGetSalH1.Rows.Count-1; i >= 0; i--)
                        {
                            ...
                            if (Convert.ToDateTime(dtGetSalH1.Rows[i]["DateAdjusted"]) <= DateTime.Today)
                            {
                ...
                string getAppraisalStr = "SELECT "+
	                                        ...
	                                        "CASE WHEN ttscore>0 and ttfactors>0 then ttscore/ttfactors ELSE CONVERT(decimal(16,2),0) END as [Total Avg Factor] "+
	                                         ...
	                                         "(  "+
                                                ...
                                                "left join SADJ1 s1 on sh.refnum=s1.refnum where sh.flag='APPR' and sh.[status]='P' and ISNULL(sh.isapproved,0)=1 and sh.empnum='" + hemph["empnum"].ToString().Trim() + "' GROUP BY sh.refnum,YEAR(sh.trandate),SH.mtotalscore " +
